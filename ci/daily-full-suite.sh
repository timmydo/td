...
=======
# TD_CHAIN_CACHE= (set-and-empty = force-cold, #317): the per-PR loop reuses the machine-wide
...
TD_CHAIN_CACHE= TD_SUBST_FORCE_BUILD=1 TD_BUILD_JOBS=${TD_BUILD_JOBS:-4} ./check.sh >"$hlog" 2>&1 || heavy_rc=$?
>>>>>>> d8d01de (flip #317: gates default to SHARED warm builder state; Private is the audited cold opt-out)
