...
# make, sed, grep — come today from guix packages (specification->package); this
...
# (one fewer Guile dep). The irreducible compiler seed (gcc-toolchain/glibc/
# binutils) stays external (§5, retired last). Per tool: STRUCTURAL (built with
# guix/Guile off PATH), DURABLE behavioral (the tool runs --version), DURABLE
# reproducibility (td-builder check double-build), MIGRATION ORACLE (distinct store
# path from guix's build — own, then diverge). Locks are the guix-built seed.
HEAVY_GATES += toolchain-no-guix
toolchain-no-guix:
	@echo ">> toolchain-no-guix: td builds make + sed + grep via build-recipe (no guix/Guile in the build path); each runs, reproducible, distinct from guix; gcc/glibc/binutils seed stays external (§5)"
	@set -euo pipefail; \
	...
	for spec in make sed grep; do \
	  ...
	  case "$$spec" in \
	    make) LD_LIBRARY_PATH="$$L" "$$ns/bin/make" --version | grep -q "GNU Make 4.4.1" || { echo "FAIL: make --version" >&2; exit 1; } ;; \
	    sed)  LD_LIBRARY_PATH="$$L" "$$ns/bin/sed"  --version | grep -q "(GNU sed) 4.9" || { echo "FAIL: sed --version" >&2; exit 1; } ;; \
	    grep) LD_LIBRARY_PATH="$$L" "$$ns/bin/grep" --version | grep -q "(GNU grep) 3.11" || { echo "FAIL: grep --version" >&2; exit 1; } ;; \
	  esac; \
	  echo "  [DURABLE behavioral] $$spec runs --version from td's own store output"; \
