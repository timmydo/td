...
build-plan:
	...
	for S in $$subjects; do \
	  ...
	  for d in $$edges; do \
	    ...
	    if grep -q "^$$d /" "$(CURDIR)/tests/$$S-no-guix.lock"; then gp=`sed -n "s/^$$d //p" "$(CURDIR)/tests/$$S-no-guix.lock" | head -1`; else gp=`sed -n "s#^[^ ]*-$$d-[^ ]* \(/gnu/store/[^ ]*\)#\1#p" "$(CURDIR)/tests/$$S-no-guix.lock" | head -1`; fi; \
