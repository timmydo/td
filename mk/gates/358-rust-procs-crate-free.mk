# rust-procs-crate-free — td builds `procs` (the process viewer, 0.14.10) with its WHOLE crate
# closure (source + 297 deps) provisioned GUIX-FREE through td's OWN cargo-proxy (cargo resolved
# + fetched it, the proxy verifying each `.crate` sha256 == the crates.io index cksum); source +
# deps interned by store-add-recursive, vendored via TD_VENDOR_DIR. Contrast rust-procs (351),
# which realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle: content-address
...
HEAVY_GATES += rust-procs-crate-free
BUILD_GATES += rust-procs-crate-free
rust-procs-crate-free:
	@echo ">> rust-procs-crate-free: td builds 'procs' (0.14.10, 297 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); procs reads /proc; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
