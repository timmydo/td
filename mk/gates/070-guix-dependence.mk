...
# td's own code against guix as the oracle (corpus-*, td-build, drv-*, store-*),
# and the `no-guix` gate (110) proves the shipped IMAGE carries no guix binary
# (RUNTIME independence). What was missing is a NUMBER for the other axis —
# BUILD-TIME independence: of everything needed to BUILD a target, how much does
# td build itself vs pull from guix? This gate records that ratio.
#
# A derivation is "td-reproducible" iff a non-perturbed tests/ts/recipe-<spec>.ts
# exists (each proven store-path-equal by a corpus-* gate in THIS ladder, so a
# green loop means recipe ⇒ proof). tests/guix-dependence.scm walks the full
# build closure (derivation prerequisite graph — lowering only, NO building) of
# two targets — the owned-recipe union and the shipped system/td.scm — and emits
# a DETERMINISTIC census, compared verbatim to tests/guix-dependence.expected.
#
...
# not re-prove reproducibility (the corpus-* gates do). It quantifies td's
