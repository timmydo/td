# rust-eza — td builds `eza` (a modern `ls` replacement) FROM SOURCE, continuing the shipped
# Rust userland (procs/fd/ripgrep/sd/eza/bat, PR #80) from guix-packaged to td-built (the
# uutils-`cat` pattern; move-off-Guile §5). `eza` 0.21.6 is built via `td-builder build-recipe`
# (buildSystem "rust", noDefaultFeatures) with its FULL 233-crate closure vendored as
# fixed-output static.crates.io fetches (sha256 == each Cargo.lock checksum), pinned in
# tests/eza.lock. eza's default `git` feature pulls git2 → libgit2/openssl C, so the recipe
# builds --no-default-features (pure Rust, no git column). The .drv is td-assembled (no guix
# (derivation …)) + realized daemon-free (no guix-daemon), guix/Guile SCRUBBED FROM PATH. The
# rustc/cargo/gcc seed is external (§5). Auto-classified self-hosted by buildSystem.
#
...
#   [DURABLE behavioral] the built `eza` lists a directory's entries — real ls behavior, not
#     just --version.
#   [DURABLE repro] td-builder check's double-build agrees the build is reproducible across the
#     whole 233-crate graph.
HEAVY_GATES += rust-eza
# Self-contained lock (tests/eza.lock) — built by the gate itself; ordered after build-recipes
# (a BUILD_GATE) so its cargo build doesn't oversubscribe the corpus fan-out.
BUILD_GATES += rust-eza
rust-eza:
	@echo ">> rust-eza: td builds 'eza' (0.21.6, 233 vendored deps, --no-default-features) from source via build-recipe (offline, guix/Guile off PATH); it lists a dir + is reproducible"
	@set -euo pipefail; \
	...
	test "$$ncrate" -ge 200 || { echo "ERROR: lock has <200 vendored .crate deps ($$ncrate) — regenerate from eza's Cargo.lock" >&2; exit 1; }; \
	scratch="$(CURDIR)/.td-build-cache/rust-eza"; mkdir -p "$$scratch/tmp" "$$scratch/b"; rm -f "$$scratch/b/"*.drv; \
	...
	sh tests/ts-emit.sh "$(CURDIR)/tests/ts/recipe-eza.ts" > "$$scratch/eza.json"; \
	test -s "$$scratch/eza.json" || { echo "ERROR: ts-emit produced no JSON" >&2; exit 1; }; \
	sd="$$scratch/b"; mkdir -p "$$sd"; \
	env -i HOME="$$scratch" TMPDIR="$$scratch/tmp" PATH="$$cu/bin" TD_BUILDER_PATH="$$TD_BUILDER_PATH" TD_BUILDER_STORE="$$TD_BUILDER_STORE" TD_BUILDER_DB="$$TD_BUILDER_DB" "$$tb" build-recipe "$$scratch/eza.json" "$$lock" "$$sd" /var/guix/db/db.sqlite > "$$scratch/bout" 2>"$$scratch/err" || { echo "FAIL: build-recipe eza build (guix/Guile off PATH):" >&2; tail -30 "$$scratch/err" >&2; exit 1; }; \
	...
	test -x "$$ns/bin/eza" || { echo "FAIL: eza build produced no 'eza' binary at $$ns/bin/eza" >&2; exit 1; }; \
	...
	if [ -n "$$hit" ]; then echo "  [STRUCTURAL] CACHE HIT — recipe unchanged, reused td's prior eza build (no rebuild): $$out"; else echo "  [STRUCTURAL] td assembled + realized the .drv (TD_VENDOR_CRATES, $$ncrate deps) with guix/Guile off PATH: $$out"; fi; \
	tree="$$scratch/tree"; rm -rf "$$tree"; mkdir -p "$$tree"; : > "$$tree/alpha.txt"; : > "$$tree/beta.log"; \
	...
	echo "  [DURABLE behavioral] the td-built 'eza' listed the directory's entries (alpha.txt + beta.log) — it works as ls"; \
	if [ -n "$$hit" ] && [ -f "$$sd/verified-reproducible" ]; then \
	  ...
	  rm -rf "$$scratch/chk"; "$$tb" check "$$sd"/*.drv "$$sd/closure.txt" "$$scratch/chk" > "$$scratch/checkout.txt" 2>"$$scratch/chk.err" \
	    || { echo "FAIL: rust-eza NOT reproducible (td-builder check):" >&2; tail -6 "$$scratch/checkout.txt" "$$scratch/chk.err" >&2; exit 1; }; \
