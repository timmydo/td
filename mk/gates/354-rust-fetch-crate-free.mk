# rust-fetch-crate-free — td builds td-fetch with its crate closure provisioned GUIX-FREE:
...
# rust/gcc toolchain seed stays guix-built (retired last). Contrast rust-fetch (348), which
# realizes the crates via `guix build /gnu/store/<hash>.crate` (the guix-daemon FOD).
#
...
HEAVY_GATES += rust-fetch-crate-free
BUILD_GATES += rust-fetch-crate-free
rust-fetch-crate-free:
	@echo ">> rust-fetch-crate-free: td builds td-fetch with crates provisioned GUIX-FREE (td-fetched + interned vendor tree, TD_VENDOR_DIR), no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
	...
	scratch="$(CURDIR)/.td-build-cache/rust-fetch-crate-free"; rm -rf "$$scratch"; mkdir -p "$$scratch/tmp" "$$scratch/sd"; \
	...
	srcinfo=`sh tests/intern-src.sh "$$tb" td-fetch-src "$(CURDIR)/fetch" "$$scratch/src" target vendor .cargo` || { echo "ERROR: intern source failed" >&2; exit 1; }; \
	eval "$$srcinfo"; \
