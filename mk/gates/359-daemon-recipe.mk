...
daemon-recipe:
	...
	grep ' /gnu/store/' "$$lock" | sed 's/^[^ ]* //' | xargs $(GUIX) build >/dev/null || { echo "ERROR: could not realize the hello seed + source (regenerate the lock on a channel bump)" >&2; exit 1; }; \
	scratch="$(CURDIR)/.td-build-cache/daemon-recipe"; chmod -R u+w "$$scratch" 2>/dev/null || true; rm -rf "$$scratch"; mkdir -p "$$scratch/tmp" "$$scratch/asm" "$$scratch/d"; \
