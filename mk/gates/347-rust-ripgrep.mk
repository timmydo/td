# rust-ripgrep — td builds `ripgrep` (the `rg` fast grep) FROM SOURCE, continuing the shipped
# Rust userland (procs/fd/ripgrep/sd/eza/bat, PR #80) from guix-packaged to td-built (the
# uutils-`cat` / `fd` pattern; move-off-Guile §5). `ripgrep` 14.1.1 (binary `rg`) is built via
# `td-builder build-recipe` (buildSystem "rust") with its FULL 57-crate closure vendored as
# fixed-output static.crates.io fetches (sha256 == each Cargo.lock checksum), pinned in
# tests/ripgrep.lock. Pure Rust at default features (jemallocator is musl-only, cfg-excluded on
# gnu; pcre2 is non-default), so no C build. run_rust assembles the cargo vendor dir + builds
# offline; the .drv is td-assembled (no guix (derivation …)) + realized daemon-free (no
# guix-daemon), guix/Guile SCRUBBED FROM PATH. The rustc/cargo/gcc seed is external (§5).
#
...
#   [DURABLE behavioral] the built `rg` finds a pattern line in a tree (and not an unrelated
...
#     whole 57-crate graph.
HEAVY_GATES += rust-ripgrep
# Self-contained lock (tests/ripgrep.lock) — built by the gate itself; ordered after
# build-recipes (a BUILD_GATE) so its cargo build doesn't oversubscribe the corpus fan-out.
BUILD_GATES += rust-ripgrep
rust-ripgrep:
	@echo ">> rust-ripgrep: td builds 'ripgrep' (rg 14.1.1, 57 vendored deps) from source via build-recipe (offline, guix/Guile off PATH); it greps a pattern + is reproducible"
	@set -euo pipefail; \
	...
	test "$$ncrate" -ge 50 || { echo "ERROR: lock has <50 vendored .crate deps ($$ncrate) — regenerate from ripgrep's Cargo.lock" >&2; exit 1; }; \
	scratch="$(CURDIR)/.td-build-cache/rust-ripgrep"; mkdir -p "$$scratch/tmp" "$$scratch/b"; rm -f "$$scratch/b/"*.drv; \
	...
	env -i HOME="$$scratch" TMPDIR="$$scratch/tmp" PATH="$$cu/bin" TD_BUILDER_PATH="$$TD_BUILDER_PATH" TD_BUILDER_STORE="$$TD_BUILDER_STORE" TD_BUILDER_DB="$$TD_BUILDER_DB" "$$tb" build-recipe "$$scratch/rg.json" "$$lock" "$$sd" /var/guix/db/db.sqlite > "$$scratch/bout" 2>"$$scratch/err" || { echo "FAIL: build-recipe ripgrep build (guix/Guile off PATH):" >&2; tail -30 "$$scratch/err" >&2; exit 1; }; \
	...
	test -x "$$ns/bin/rg" || { echo "FAIL: ripgrep build produced no 'rg' binary at $$ns/bin/rg" >&2; exit 1; }; \
	...
	if [ -n "$$hit" ]; then echo "  [STRUCTURAL] CACHE HIT — recipe unchanged, reused td's prior ripgrep build (no rebuild): $$out"; else echo "  [STRUCTURAL] td assembled + realized the .drv (TD_VENDOR_CRATES, $$ncrate deps) with guix/Guile off PATH: $$out"; fi; \
	...
	echo "  [DURABLE behavioral] the td-built 'rg' found the 'needle' line (and not the unrelated file) — it works as ripgrep"; \
	if [ -n "$$hit" ] && [ -f "$$sd/verified-reproducible" ]; then \
	  ...
	  rm -rf "$$scratch/chk"; "$$tb" check "$$sd"/*.drv "$$sd/closure.txt" "$$scratch/chk" > "$$scratch/checkout.txt" 2>"$$scratch/chk.err" \
	    || { echo "FAIL: rust-ripgrep NOT reproducible (td-builder check):" >&2; tail -6 "$$scratch/checkout.txt" "$$scratch/chk.err" >&2; exit 1; }; \
