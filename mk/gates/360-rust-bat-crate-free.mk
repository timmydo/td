# rust-bat-crate-free — td builds `bat` (the cat replacement, 0.25.0, no git/onig C) with its
# WHOLE crate closure (source + 207 deps) provisioned GUIX-FREE through td's OWN cargo-proxy
# (cargo resolved + fetched it, the proxy verifying each `.crate` sha256 == the crates.io index
# cksum); source + deps interned by store-add-recursive, vendored via TD_VENDOR_DIR. Contrast
# rust-bat (353), which realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle:
# content-address (Cargo.lock pin == index cksum) is the oracle. Shared build+assert in
# tests/crate-free-build.sh. The rust/gcc toolchain seed stays guix-built (retired last). This
# completes the shipped Rust userland (procs/fd/ripgrep/sd/eza/bat) built guix-free.
#
...
HEAVY_GATES += rust-bat-crate-free
BUILD_GATES += rust-bat-crate-free
rust-bat-crate-free:
	@echo ">> rust-bat-crate-free: td builds 'bat' (0.25.0, 207 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); bat prints a file; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
