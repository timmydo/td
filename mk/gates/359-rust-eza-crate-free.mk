# rust-eza-crate-free — td builds `eza` (the ls replacement, 0.21.6, --no-default-features) with
# its WHOLE crate closure (source + 233 deps) provisioned GUIX-FREE through td's OWN cargo-proxy
...
# rust-eza (352), which realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle:
...
HEAVY_GATES += rust-eza-crate-free
BUILD_GATES += rust-eza-crate-free
rust-eza-crate-free:
	@echo ">> rust-eza-crate-free: td builds 'eza' (0.21.6, 233 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); eza lists a dir; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
