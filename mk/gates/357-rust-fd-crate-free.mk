# rust-fd-crate-free — td builds `fd` (the fast find, fd-find 10.2.0) with its WHOLE crate
# closure (source + 113 deps) provisioned GUIX-FREE through td's OWN cargo-proxy: cargo resolved
# + fetched it through `td-feed cargo-proxy` (tools/warm-cargo-proxy.sh fd-find 10.2.0 fd, host
# PREP — the crates.io name `fd-find` differs from the recipe name `fd`), the proxy verifying each
# `.crate` sha256 == the crates.io index cksum (UPSTREAM pin, NOT a guix artifact); source + deps
# interned by store-add-recursive, vendored via TD_VENDOR_DIR. Contrast rust-fd (346), which
# realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle: content-address
# (Cargo.lock pin == index cksum) is the oracle. Shared build+assert in tests/crate-free-build.sh.
# The rust/gcc toolchain seed stays guix-built (retired last).
#
...
HEAVY_GATES += rust-fd-crate-free
BUILD_GATES += rust-fd-crate-free
rust-fd-crate-free:
	@echo ">> rust-fd-crate-free: td builds 'fd' (fd-find 10.2.0, 113 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); fd finds a file; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
