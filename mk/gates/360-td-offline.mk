...
td-offline:
	...
	ip=`$(GUIX) build iproute2 2>/dev/null | head -1`/sbin/ip; \
	unshare=`$(GUIX) build util-linux 2>/dev/null | sed -n 's,$$,/bin/unshare,p' | while read c; do test -x "$$c" && { echo "$$c"; break; }; done`; \
