...
# ITSELF — its OWN build daemon realizes the TD-BUILT hello `.drv` TWICE in two
# independent user-namespace sandboxes (the #25 executor, via `realize_drv`) and compares
# the per-output NAR hashes (the #21/S2 NAR serializer + SHA-256): equal ⇒ reproducible,
# with NO guix-daemon and NO `guix build --check` anywhere.
#
...
#   • VERDICT: cache-lib `cached_check` submits a daemon CHECK — the daemon realizes the
#     SAME `.drv` twice and compares NAR hashes (td's own double-build), no `guix build
#     --check`.
...
td-check:
	...
	echo ">> td's OWN reproducibility verdict: the daemon realizes the .drv TWICE and compares per-output NAR hashes (no guix build --check)"; \
	...
	echo "PASS: td computed the reproducibility verdict ITSELF — its OWN build daemon realized the TD-ASSEMBLED hello .drv (builder = bootstrapped stage0, guix/Guile off PATH) TWICE in independent userns sandboxes to a byte-identical NAR, and the reproducible binary runs ('Hello, world!'). No guix-daemon, no guix repl, no guix gc, no guix build --check anywhere — the removable guix differential oracles were dropped (directive 3); td's own double-build verdict is the whole gate."
