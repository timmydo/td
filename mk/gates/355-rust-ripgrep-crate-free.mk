# rust-ripgrep-crate-free — td builds `ripgrep` (rg 14.1.1) with its WHOLE crate closure (source
# + 57 deps) provisioned GUIX-FREE through td's OWN cargo-proxy: cargo resolved + fetched it
# through `td-feed cargo-proxy` (tools/warm-cargo-proxy.sh, host PREP), the proxy verifying each
# `.crate` sha256 == the crates.io sparse-index cksum (the UPSTREAM pin, NOT a guix artifact);
# source + deps are interned by td's OWN store-add-recursive and build-recipe vendors from them
# (TD_VENDOR_DIR). PoC scaling the guix-free crate path (#163) to a real corpus rust package.
# Contrast rust-ripgrep (347), which realizes the crates via `guix build /gnu/store/<h>.crate`.
# No guix oracle (human 2026-06-23): content-address (the Cargo.lock pin == the index cksum) is
# the oracle. The shared build+assert lives in tests/crate-free-build.sh; this gate adds the
# package-specific behavioral leg. The rust/gcc toolchain seed stays guix-built (retired last).
#
...
rust-ripgrep-crate-free:
	@echo ">> rust-ripgrep-crate-free: td builds 'ripgrep' (rg 14.1.1, 57 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); rg greps a needle; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
