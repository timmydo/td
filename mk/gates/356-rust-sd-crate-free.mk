# rust-sd-crate-free — td builds `sd` (the find-and-replace tool, 1.0.0) with its WHOLE crate
# closure (source + 111 deps) provisioned GUIX-FREE through td's OWN cargo-proxy: cargo resolved
# + fetched it through `td-feed cargo-proxy` (tools/warm-cargo-proxy.sh, host PREP), the proxy
# verifying each `.crate` sha256 == the crates.io index cksum (UPSTREAM pin, NOT a guix artifact);
# source + deps interned by store-add-recursive, vendored via TD_VENDOR_DIR. Contrast rust-sd
# (349), which realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle: the
...
HEAVY_GATES += rust-sd-crate-free
BUILD_GATES += rust-sd-crate-free
rust-sd-crate-free:
	@echo ">> rust-sd-crate-free: td builds 'sd' (1.0.0, 111 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); sd find-and-replaces; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
