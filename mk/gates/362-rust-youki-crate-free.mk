# rust-youki-crate-free — td builds `youki` (the Rust OCI container runtime, 0.6.0) with its
# WHOLE crate closure (source + 663 deps) provisioned GUIX-FREE through td's OWN cargo-proxy:
# cargo resolved + fetched it through `td-feed cargo-proxy` (tools/warm-cargo-proxy.sh youki
# 0.6.0, host PREP), the proxy verifying each `.crate` sha256 == the crates.io index cksum;
# source + deps interned by store-add-recursive, vendored via TD_VENDOR_DIR. Contrast rust-youki
# (344), which realizes the crates via `guix build /gnu/store/<h>.crate`. No guix oracle:
...
HEAVY_GATES += rust-youki-crate-free
BUILD_GATES += rust-youki-crate-free
rust-youki-crate-free:
	@echo ">> rust-youki-crate-free: td builds 'youki' (0.6.0, 663 deps) GUIX-FREE via the cargo-proxy (interned vendor tree, TD_VENDOR_DIR); youki runs as an OCI runtime CLI; reproducible; no guix build / no /gnu/store crate / no oracle"
	@set -euo pipefail; \
