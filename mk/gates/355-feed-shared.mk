# feed-shared — a persistent, SHARED td-feed daemon (tools/feed-ensure.sh) serves a warmed
...
#
#   [DURABLE behavioral] feed-ensure starts ONE shared daemon; two consumers in DIFFERENT
...
#   [DURABLE structural] a 2nd feed-ensure REUSES the running daemon (same addr + pid), so
...
feed-shared: td-feed
	...
	faddr=`sh tools/feed-ensure.sh`; fpid=`cat "$$TD_FEED_DIR/feed.pid"`; \
	test -n "$$faddr" -a -n "$$fpid" || { echo "ERROR: feed-ensure did not start a shared daemon" >&2; exit 1; }; \
	echo "  [DURABLE behavioral] feed-ensure started ONE shared daemon at $$faddr (pid $$fpid)"; \
	...
	faddr2=`sh tools/feed-ensure.sh`; fpid2=`cat "$$TD_FEED_DIR/feed.pid"`; \
	test "$$faddr" = "$$faddr2" -a "$$fpid" = "$$fpid2" || { echo "FAIL: feed-ensure restarted the daemon ($$faddr/$$fpid -> $$faddr2/$$fpid2) instead of reusing it" >&2; exit 1; }; \
	echo "  [DURABLE structural] a 2nd feed-ensure REUSED the same daemon ($$faddr2 pid $$fpid2) — persistent + shared, not per-run"; \
