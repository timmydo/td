...
Landing 1 (this one) implements the crate: table, validator, ordering,
readiness, backoff, supervision, the `/proc` layer, and the event loop. It
is not yet wired into the image — `/etc/td-svc.conf` is not generated and
PID 1 still runs the eleven-line inittab. The control socket, ordered
shutdown, Ctrl-Alt-Del arming, and log capture land subsequently, in that
order. Sections above describe the completed design; anything not yet built
is specified here so the later landings implement a reviewed target rather
than an improvised one.

Two things the next landing must resolve before it can be correct, both
found in review and recorded above rather than left to be rediscovered:
the shipped greeter's containment is its wrapper process alone (§4,
"Stopping"), which is safe but cannot reach the login tree; and `log=` /
`console=` are refused by name until log capture exists, so the
