...
- [ ] **ts-frontend** — claimed claude-fable-3ca5dd 2026-06-13 (Phase 1 of §5 move-off-Guile: TypeScript spec surface lowering to the frozen oracle's drvs; charter landed #15. Sub-tasks 1-2 DONE: `ts` rung (pinned tsc type-checks + emits v0 spec, vr×3) and `ts-eval` rung (pure-Rust boa evaluator — vendored offline/hash-pinned, --check reproducible, curated global removes Date + denies Math.random, vr×2). Decisions (human 2026-06-13): boa vendored as a pinned input; swc CLI is a stub so tsc does the transpile too. Next: sub-tasks 3-5 hermetic probes + pkg/storeRef lowering → NAR-hash differential) — `plan/ts-frontend.md`
