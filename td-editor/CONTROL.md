...
Its framing, envelope, codecs and response lines are td-ui's `control`
module, and the private listener and the bounded transport thread that
hands typed requests to the native window's UI thread are td-ui's
`control_socket` and `control_worker`, specified under "Driving" in
[td-ui/DESIGN.md](../td-ui/DESIGN.md#driving). The editor's `Request` is
the worker's `Parse` type; the window's turn loop consumes its typed jobs.

## Framing

One frame is a four-byte unsigned big-endian length followed by that many
payload bytes, 1..=1,048,576 bytes for requests and responses, with no
newline terminator. The frame encoder, the poisoning one-frame `Decoder`
and their exact refusal and allocation rules are td-ui's (`td_ui::control`,
under "Framing" in its design); `td_editor::control` re-exports `frame`,
`Decoder` and `MAX_FRAME` for the editor's replay and tests. The worker
owns deadlines, connection limits, request/response queues and
cancellation; the framing alone claims none of those safeguards. Replay
runs td-ui's consecutive-frame runner over the editor's session; the
one-frame decoder serves the one-request-per-connection worker.
...
Envelope validation, error-ID recovery and the field codecs are td-ui's
(`envelope`, `decimal`, `size`, `boolean`, `hex`, `unhex`), shared with
replay, while the two adapters retain distinct command allowlists.
...
and both response lines are rendered by td-ui's `ok` and
`Refusal::response`: the editor's `Refusal` is the toolkit's
`Refusal<Error>`, and the transport's own `protocol` and `limit` map onto
the editor's codes of the same name.
...
Both scrollbars accept the same fenced pointer press/move/release
...
The horizontal bar is present for unwrapped text and directory tabs,
reserving 16 logical pixels above the status bar. Wrapping hides it and
resets horizontal origin. Read the current view grid after wrap changes;
horizontal drags change columns only and preserve row/selection/revision.
...
state/text responses through replay, immutable controller state, maximum pages
and 64-tab output, stale-after-Undo rejection and arbitrary byte input as
correctly framed payloads through the editor's grammar; every frame split,
single-byte delivery, premature EOF, zero/oversized/trailing frames, poisoned
decoder behavior and raw arbitrary headers are td-ui's framing tests. Invalid
envelopes/read-only command refusals are compared with replay, separately from
the intentionally different mutation allowlists/selection guards.
...
These library tests need no display, socket, dictionary file or external
process.

## Private socket publication and bounded worker

The private listener the editor publishes for `--control-socket PATH` and the
bounded transport thread that owns it are td-ui's `control_socket` and
`control_worker`, moved from this crate intact; their complete path/trust
contract, same-UID race boundary, connection, deadline and memory bounds, and
their kernel and deterministic tests, are specified under "Driving" in
[td-ui/DESIGN.md](../td-ui/DESIGN.md#driving). What remains the editor's: the
option is opt-in and literal, an absolute non-NUL path of at most 107 bytes
given once; `Request::parse` is the worker's `Parse` type, so only the
query/edit subset above reaches the window, and parsing retains a fixed-size
descriptor plus at most 256 KiB of decoded Insert text with text redacted from
a job's debug output (UI insertion clones that bounded text for the owned
controller command, and the model separately normalizes and adopts it within
its existing edit budgets); the window's `end_turn` polls at most two jobs per
outer turn (`CONTROL_JOBS_PER_TURN`), holds at most eight `wait-frame` jobs
and refuses the ninth with `limit`, answers through `Job::respond_with` after
its own modal/target admission, and closes the worker with the window,
reporting a failed shutdown. The window's confinement test pins that polling
contract; td-ui's pins the transport's. The crate keeps its own
`trusted-test-root` declaration: its window tests bind real sockets under the
same 0700 fixtures, whose ancestry the gate's namespace would otherwise leave
unmapped, and its file tests use that private root too (DEVELOPMENT.md,
