...
/// The editor owns no Wayland objects of its own: the seat's devices and
/// the clipboard's are the client's.
...
/// The editor's half of the clipboard: the text behind the client's live
/// source, and the transfers in flight.
#[derive(Default)]
struct Clipboard {
...
/// The editor's pointer gesture over the client's pointer: the last
/// position, the held left button and the wheel accumulator.
#[derive(Default)]
struct Pointer {
    x: i32,
    y: i32,
...
struct Window {
    ...
    path_completion: Option<crate::path_completion::Worker>,
    last_completion: u64,
...
struct PathPrompt {
    ...
    completion: crate::path_completion::State,
}
struct PathIdentity {
    id: u64,
...
impl PathPrompt {
    fn completion_notice(
        ...
        let header = if !paused.is_empty() {
            ...
            "Save before close; Esc/C-g cancels ALL closing".into()
        } else {
            format!("{action}: Enter submits; Esc cancels; Tab/Up/Down chooses")
        };
        let escaped: String = self.text.chars().flat_map(char::escape_debug).collect();
        let start = escaped
            .char_indices()
            .rev()
    ...
    fn notice(&self) -> String {
        ...
        let action = match &self.action {
            ...
            PathAction::Copy(source) | PathAction::Rename(source) => {
...
impl Window {
    ...
    /// The client's clipboard outcomes: the editor's half of each.
    fn clipboard_event(&mut self, event: ClipboardEvent) -> Result<()> {
        match event {
            ...
            ClipboardEvent::Send(right) => {
                ...
                // The client's live source and this text are set together by
                // `offer_clipboard` and cleared together on `Cancelled` and
                // release, so a send without text is the editor's own fault.
                let text = self
                    .clipboard
                    .text
                    .clone()
                    .ok_or("clipboard source disappeared")?;
                match crate::transfer::Outgoing::begin(right, text, self.clock) {
            ...
            ClipboardEvent::Cancelled => self.clipboard.text = None,
    ...
    /// The client released the keyboard, or the seat with it: the editor
    /// drops what focus and a map allowed and keeps every document.
    fn keyboard_lost(&mut self) -> Result<()> {
        self.searches.cancel_wrap();
    ...
    /// The client released the pointer: the gesture ends without touching
    /// keyboard focus or the selection.
    fn pointer_lost(&mut self) -> Result<()> {
    ...
    /// The live pointer's event, decoded by the client, which showed the
    /// pointer image on enter and keeps the enter serial.
    fn pointer_event(&mut self, event: td_ui::pointer::Event) -> Result<()> {
    ...
    /// The live keyboard's event, which the client already applied to its
    /// map, focus, snapshot and held keys.
        ...
        match event {
            KeyboardEvent::Keymap(result) => {
                ...
                match result {
                    Ok(()) => {
                        self.ui
                            .dispatch(Event::Focus(self.client.input().focused))
                            .map_err(error)?;
                        self.notify(
                            "Keymap compiled; waiting for focus/modifier snapshot. \
            ...
            KeyboardEvent::Focus(true) => {
                self.ui
                    .dispatch(Event::Focus(self.client.input().map.is_some()))
                    .map_err(error)?;
                self.frames.invalidate(true);
            }
            ...
            && matches!(chord, "w" | "s" | "S" | "R" | "C" | "d" | "u" | "x" | "+" | "q")
        {
            if !repeated {
                if chord == "q" {
                    ...
                    self.copy_request(tab, revision);
    ...
    fn tick(&mut self, now: u64, repeat: bool) -> Result<()> {
            ...
            .and_then(|files| files.poll(&mut self.ui))
        {
            ...
            if let Some(job) = self.control_file_job.take() {
                ...
                let outcome = match job {
                    ...
                    ControlFile::Copy(id) => self
                        .control_jobs
                        .copied(id, result.as_ref().map(|_| ()).map_err(|error| error.code)),
                };
        ...
        let wait = Duration::from_millis(self.client.wait_ms(now));
    ...
    fn control_copy_job(
        ...
        self.ui
            ...
            .ok_or(crate::Error::Exhausted)?;
        let files = self.files.as_mut().ok_or(crate::Error::Unavailable)?;
        let id = self.control_jobs.begin_copy(target.tab, target.revision)?;
        let notice = match files.copy(&self.ui, target.tab, target.revision, source, name) {
            Ok(()) => {
                self.control_file_job = Some(ControlFile::Copy(id));
                "Copy pending; unsaved buffers are not copied".into()
            }
            Err(detail) => {
                self.control_jobs
                    .copied(id, Err(crate::Error::Unavailable))?;
                format!("Copy refused: {detail}")
            }
        };
        self.stop_pointer();
        self.control_mutation_accepted();
    ...
    fn control_rename_job(
        ...
        let id = self
            .control_jobs
            .begin_rename(target.tab, target.revision)?;
        let notice = match files.rename(&self.ui, target.tab, target.revision, source, name) {
            Ok(()) => {
                self.control_file_job = Some(ControlFile::Rename(id));
                "Rename pending; open tabs will follow the new path".into()
            }
            Err(detail) => {
                self.control_jobs
                    .renamed(id, Err(crate::Error::Unavailable))?;
    ...
    fn control_path_answer(
        ...
        let id = match &prompt.action {
            ...
            PathAction::Copy(source) => {
                self.control_copy_job(target, source.clone(), path.into_os_string())?
            }
    ...
    fn control_clipboard_state(&self) -> crate::Result<String> {
        let selection = match self.client.selection() {
            None => "none",
            Some(_) => match self.client.selection_mime() {
                Some(mime) if mime.eq_ignore_ascii_case(UTF8) => "utf8",
                Some(mime) if mime.eq_ignore_ascii_case(PLAIN) => "plain",
                _ => "unsupported",
            },
        };
        let source = self
...
impl Window {
    fn open_menu(&mut self, group: crate::menu::Group) -> Result<()> {
        ...
        let mut menu = crate::menu::Menu {
            ...
            paste: self.client.input().focused
                && self.client.clipboard()
    ...
    fn conflict_notice(&self) -> Option<String> {
        ...
        if self.client.keyboard().is_none() || self.client.input().map.is_none() {
            return Some(format!(
                "{identity}\nInput unavailable.\nRestore the seat/keymap."
            ));
        }
        if !self.client.input().focused || !self.client.input().synchronized {
    ...
    fn closing_notice(&self) -> Option<String> {
        ...
        } else if !self.client.input().focused || !self.client.input().synchronized {
    ...
    fn path_notice(&self) -> Option<String> {
        ...
        if !matches!(prompt.completion, crate::path_completion::State::Idle) {
            let paused = if self.client.keyboard().is_none() || self.client.input().map.is_none() {
                "Path entry paused: restore keyboard/keymap"
            } else if !self.client.input().focused || !self.client.input().synchronized {
                "Path entry paused: focus editor; tap/release Shift"
            } else {
                ""
            };
    ...
    fn copy_request(&mut self, tab: crate::model::TabId, revision: u64) {
        let prepared = (|| -> Result<(crate::files::RenameSource, PathIdentity)> {
            let files = self.files.as_ref().ok_or("Not a file window")?;
            if files.busy() {
                return Err("File operation pending; wait before copying".into());
            }
            let doc = self.ui.editor().document(tab).map_err(error)?;
            let row = doc
                .text()
            ...
            let source = files
                .directory(tab)
                .and_then(|directory| directory.copy_source(row))
                .ok_or("Select a regular file; directories and links are not copied")?;
            let id = self
                .last_dialog_id
                .checked_add(1)
                .ok_or("Dialog counter exhausted")?;
        ...
        match prepared {
            ...
            Err(detail) => self.notify(format!("Copy refused: {detail}")),
        }
    }
    ...
    fn rename_request(&mut self, tab: crate::model::TabId, revision: u64) {
        let prepared = (|| -> Result<(crate::files::RenameSource, PathIdentity)> {
            let files = self.files.as_ref().ok_or("Not a file window")?;
            if files.busy() {
                return Err("File operation pending; wait before renaming".into());
            }
            let doc = self.ui.editor().document(tab).map_err(error)?;
            let row = doc
                .text()
            ...
            let source = files
                .directory(tab)
                .and_then(|directory| directory.rename_source(row))
        ...
        match prepared {
            Ok((source, identity)) => {
                ...
                self.client.cancel_repeat();
                self.prompt = Some(PathPrompt {
                    action: PathAction::Rename(source),
                    text: String::new(),
                    identity: Some(identity),
                    completion: crate::path_completion::State::Idle,
                });
            ...
            Err(detail) => self.notify(format!("Rename refused: {detail}")),
        }
    }
        ...
        match chord {
            ...
            "Return" => {
                ...
                if let PathAction::Copy(source) = &prompt.action {
                        ...
                        || self.control_file_job.is_some()
                    {
                        self.prompt = Some(prompt);
                        self.notify(
                            "File operation pending; copy name retained. Try Return again.",
                        );
                        return;
                    }
                    let result = prompt
                    ...
                    if let Err(detail) = result {
                        self.notify(format!("Copy refused: {detail}"));
                    }
                    return;
                }
                ...
                if let PathAction::Rename(source) = &prompt.action {
                    ...
                    if let Err(detail) = result {
                        self.notify(format!("Rename refused: {detail}"));
                    }
                    return;
                }
                let Some(files) = &mut self.files else {
                ...
                let result = match prompt.action {
                    ...
                    PathAction::Copy(_) => Err("Copy requires a pinned source".into()),
                };
            ...
            "Tab" | "S-Tab" | "Up" | "Down" | "PageUp" | "PageDown" => {
                ...
                match &mut prompt.completion {
                    ...
                    _ if matches!(chord, "Tab" | "S-Tab") => {
                        if let Err(detail) = self.request_path_completion(&mut prompt) {
                            prompt.completion = crate::path_completion::State::Failed(detail);
                        }
                    }
                    _ => {}
                }
...
impl Window {
    ...
    /// Focus loss: the client retired the selection; a paste in flight
    /// is cancelled.
    fn clipboard_focus_lost(&mut self) {
        if self.clipboard.incoming.take().is_some() {
            self.notify("Paste cancelled: clipboard focus lost.");
        }
    }

    /// The client released the data device, with its seat or its
    /// manager: the transfers end, the text behind the source is dropped
    /// and the menu closes.
    ...
    /// A fresh source becomes the selection at `serial`; the text behind
    /// it is retained for the sends, and a paste in flight is dropped.
    fn offer_clipboard(&mut self, text: std::sync::Arc<str>, serial: u32) -> Result<()> {
        self.client.offer_selection(serial)?;
...
mod tests {
    ...
    fn seat_fixture() -> (Window, UnixStream, u32) {
        ...
        assert!(w.client.keyboard().is_none());
        w.event(message(seat, 0, &[3])).unwrap();
        let device = w.client.keyboard().unwrap();
        assert_eq!(
            drain(&b).0,
            [
                message(seat, 0, &[w.client.pointer().unwrap()]),
    ...
    fn pointer_shift_selection_and_capability_replacement_are_independent_of_keyboard() {
        ...
        let old = w.client.pointer().unwrap();
        let seat = w.client.seat().unwrap();
        w.event(message(seat, 0, &[2])).unwrap();
        assert!(w.client.pointer().is_none());
        assert!(w.ui.focused());
        assert!(w.client.input().focused && w.client.input().synchronized);
        let selection = w.ui.editor().document(tab).unwrap().selection();
        w.event(message(old, 2, &[0, 0, 0])).unwrap();
        assert_eq!(w.ui.editor().document(tab).unwrap().selection(), selection);
        assert!(w.event(message(old, 3, &[0, 0, 0x110, 2])).is_err());
        w.event(message(seat, 0, &[3])).unwrap();
        assert_ne!(w.client.pointer(), Some(old));
        ...
        pointer_enter(&mut w);
        w.client.input_mut().synchronized = false;
    ...
    fn native_find_open_cancels_prefix_drag_repeat_and_pending_paste_and_rejects_multiline_seed() {
        ...
        w.client.input_mut().key(106, true).unwrap();
        w.client.arm(106, 0);
    ...
    /// A window whose registry offers a v9 data-device manager beside the
    ...
    fn clipboard_fixture() -> (Window, UnixStream, u32, u32) {
        ...
        for event in [
            ...
            global(3, "xdg_wm_base", 1),
            global(4, "wl_seat", 10),
            global(8, "wl_data_device_manager", 9),
        ] {
            w.event(event).unwrap();
        }
        w.event(message(SYNC, 0, &[0])).unwrap();
        let seat = w.client.seat().unwrap();
        assert!(w.client.clipboard() && !w.client.is_required(8));
        let device = 12;
        assert!(drain(&peer).0.contains(&message(11, 1, &[device, seat])));
        w.event(message(seat, 0, &[3])).unwrap();
    ...
    fn copy_file_path_refuses_untitled_non_utf8_and_stale_target_without_replacing_source() {
        ...
        let old = (w.client.source(), w.clipboard.text.clone());
        drain(&peer);
        ...
        assert_eq!((w.client.source(), w.clipboard.text.clone()), old);
        assert!(drain(&peer).0.is_empty());
    ...
    fn source_send(w: &mut Window, peer: &UnixStream, source: u32, mime: &str, file: &File) {
        ...
        assert!(w.client.needs_descriptor(&event).unwrap());
        w.event(event).unwrap();
    }
    ...
    fn clipboard_received_socket_is_nonblocking_bounded_and_serves_immutable_bytes() {
        ...
        source_send(&mut w, &peer, source, UTF8, &writer);
        drop(writer);
    ...
    fn clipboard_paste_cancels_on_focus_escape_edit_selection_and_deadline() {
        for action in 0..5 {
            let (mut w, peer, keyboard, device) = clipboard_fixture();
            selection_offer(&mut w, device, 0xff00_0010, &[PLAIN]);
            w.clipboard_request("paste", 1, 0).unwrap();
            let (_, mut files) = drain(&peer);
            let mut writer = files.pop().unwrap();
    ...
    fn clipboard_selection_before_keyboard_enter_is_retained_but_cannot_paste_unfocused() {
        ...
        assert!(w.clipboard.incoming.is_none());
        assert!(drain(&peer).0.is_empty());
        assert_eq!(w.client.selection(), Some(0xff00_0010));
        assert!(w.client.selection_mime().is_some());
        focus(&mut w, keyboard);
        w.clipboard_request("paste", 1, 0).unwrap();
    ...
    fn clipboard_busy_unsupported_and_retired_sends_close_their_own_rights() {
        ...
        source_send(&mut w, &peer, source, PLAIN, &first_writer);
        drop(first_writer);
        assert!(w.clipboard.outgoing.is_some());
    ...
    #[test]
    fn clipboard_over_long_send_mime_drops_its_right_without_disconnecting() {
        let (mut w, peer, keyboard, _device) = clipboard_fixture();
        w.event(message(keyboard, 4, &[0, 4, 0, 0, 0])).unwrap();
        key(&mut w, keyboard, 46);
        let source = w.client.source().unwrap();
    ...
    fn file_prompts_save_snapshots_and_keep_window_alive_during_io() {
        for profile in [Profile::Windows, Profile::Emacs] {
            ...
            assert!(w.client.repeat(w.clock + 1000).unwrap().is_some());
            assert!(w.ui.editor().document(tab).unwrap().dirty());
            assert!(w
                .notice
                .as_ref()
    ...
    fn remote_prompt_answers_complete_find_and_replace_without_input_readiness() {
        ...
        assert!(!w.client.input().focused && !w.client.configured());
        assert!(w.client.keyboard().is_none());
    }
    ...
    fn clipboard_state_classifies_offers_and_retained_sources_without_input() {
        ...
        w.event(message(REGISTRY, 1, &[8])).unwrap();
        w.clipboard.text = Some(std::sync::Arc::from("private λ"));
        w.client.input_mut().focused = false;
        w.search_request("find", 1, 0).unwrap();
        let snapshot = clipboard_snapshot(&mut w);
        assert!(snapshot.contains("\tdevice=0\tfocus=0\tselection=none\t"));
    ...
    fn prompt_state_keeps_path_text_and_live_dialog_authority_in_one_snapshot() {
        ...
        for (name, kind) in [
            ...
            assert!(answer.ends_with(&format!("{}\tcompletion=idle", w.control_dialog_fields())));
    ...
    fn remote_wheel_matches_native_diagonal_scrolling_and_wrap_clamping() {
        for wrap in [false, true] {
            ...
            for (rows, columns) in [(1i32, 1i32), (-1, 0), (0, -1), (100, 100), (-100, -100)] {
                ...
                let pointer = native.client.pointer().unwrap();
                native
    ...
    fn remote_wheel_refuses_menus_and_missing_native_readiness_without_side_effects() {
        for guard in [
            ...
            match guard {
                ...
                "enter" => {
                    let pointer = w.client.pointer().unwrap();
                    w.event(message(pointer, 1, &[1, SURFACE])).unwrap();
                }
                "device" => {
                    let seat = w.client.seat().unwrap();
                    w.event(message(seat, 0, &[2])).unwrap();
    ...
    fn remote_wheel_even_zero_cancels_drag_repeat_and_fractional_physical_wheel() {
        ...
        assert!(w.client.repeat(w.clock + 1000).unwrap().is_none());
        assert_eq!((w.pointer.x, w.pointer.y), coordinates);
        assert!(w.client.entered().is_some() && w.activation_serial.is_none());
        w.event(message(pointer, 4, &[0, 0, 256])).unwrap();
        w.event(message(pointer, 5, &[])).unwrap();
        ...
        for profile in [Profile::Windows, Profile::Emacs] {
            for horizontal in [false, true] {
                for directory in [false, true] {
                    ...
                    for row in 0..80 {
                        std::fs::write(
                            fixture.path(&format!("file{row:02}{}", "x".repeat(120))),
                            b"body",
                        )
                        .unwrap();
                    }
                    ...
                    for w in [&mut remote, &mut native] {
                        ...
                        if directory {
                            ...
                            w.ui.dispatch(Event::Load(
                                format!("{}\n", "x".repeat(150)).repeat(80).as_bytes(),
                            ))
                            .unwrap();
                            if horizontal {
                                w.ui.dispatch(Event::Wrap {
                    ...
                    let bar = if horizontal {
                        remote
                            .ui
                            .geometry()
                            .horizontal_scrollbar(state.metrics.columns, 0)
                    } else {
                        remote.ui.geometry().scrollbar(state.metrics.rows, 0)
                    }
                    .unwrap();
                    for (phase, x, y) in [
                        ("press", bar.thumb.x + 2, bar.thumb.y + 2),
                        ("move", if horizontal { 600 } else { 50 }, 400),
                        ("release", if horizontal { 2000 } else { 50 }, 800),
                    ] {
                        remote_pointer(&mut remote, phase, x, y, false);
                        pointer_move(&mut native, x, y);
                        if phase != "move" {
                    ...
                    assert_eq!(
                        if horizontal {
                            remote.ui.tab_view(tab).unwrap().viewport.origin().column
                        } else {
                            remote.ui.tab_view(tab).unwrap().viewport.origin().row
                        },
                        if horizontal {
                            state.metrics.columns - remote.ui.geometry().grid().0
                        } else {
    ...
    #[test]
    fn horizontal_thumb_subpixel_release_does_not_round_up_into_a_drag() {
        let (mut w, _peer) = file_dialog_fixture();
        configure(&mut w, 800, 600);
        w.ui.dispatch(Event::Load("x".repeat(10000).as_bytes()))
            .unwrap();
        w.ui.dispatch(Event::Wrap {
            tab: 2,
            revision: 0,
        ...
        .unwrap();
        let bar = w.ui.geometry().horizontal_scrollbar(10001, 137).unwrap();
    ...
    fn remote_keys_require_real_input_readiness_and_never_borrow_a_press_serial() {
        for guard in [
            ...
            match guard {
                ...
                "map" => w.client.input_mut().map = None,
                "focus" => w.client.input_mut().focused = false,
                "synchronized" => w.client.input_mut().synchronized = false,
    ...
    #[test]
    fn file_copy_prompt_fences_cancel_repeat_and_native_submission() {
        for profile in [Profile::Windows, Profile::Emacs] {
            let dir = DialogDirectory::new();
            std::fs::write(dir.path("source"), b"disk").unwrap();
            let (mut w, _peer) = file_dialog_fixture();
            configure(&mut w, 800, 600);
            w.ui.dispatch(Event::Profile(profile)).unwrap();
            w.files.as_mut().unwrap().open(dir.0.clone()).unwrap();
            finish_file(&mut w);
            w.chord("C", true).unwrap();
            assert!(w.prompt.is_none());
            w.chord("C", false).unwrap();
            let id = w.last_dialog_id;
            assert!(w.control_dialog_fields().contains("path-copy"));
            ...
            w.control_file_job = Some(ControlFile::Copy(99));
            w.path_chord("Return", false);
            ...
            assert!(w
                ...
                .fields()
                .unwrap()
                .contains("job=1,copy,2,0,0,complete,-"));
            assert_eq!(std::fs::read(dir.path("n")).unwrap(), b"disk");
            assert_eq!(std::fs::read(dir.path("source")).unwrap(), b"disk");
            w.chord("C", false).unwrap();
            w.prompt.as_mut().unwrap().text = "n".into();
            w.path_chord("Return", false);
            finish_file(&mut w);
            assert!(w
                ...
                .fields()
                .unwrap()
                .contains("job=2,copy,2,1,0,error,unavailable"));
            assert_eq!(w.ui.editor().document(2).unwrap().revision(), 1);
        }
    }

    #[test]
    fn file_copy_nonregular_selection_and_invalid_names_do_not_start_io() {
        let dir = DialogDirectory::new();
        std::fs::create_dir(dir.path("child")).unwrap();
        let (mut w, _peer) = file_dialog_fixture();
        w.files.as_mut().unwrap().open(dir.0.clone()).unwrap();
        ...
        w.chord("C", false).unwrap();
        assert!(w.prompt.is_none());
        assert!(w.notice.as_ref().unwrap().contains("Select a regular file"));
        std::fs::write(dir.path("file"), b"disk").unwrap();
        ...
        for name in ["", ".", "..", "/", "a/", "nul\0name"] {
            ...
            assert!(w
                ...
                .fields()
                .unwrap()
                .contains(&format!("job={id},copy,2,1,0,error,unavailable")));
        }
    ...
    fn rename_native_invalid_destination_retains_prompt_without_a_job() {
        ...
        w.chord("R", false).unwrap();
        w.prompt.as_mut().unwrap().text = "x".repeat(4096);
        w.path_chord("Return", false);
        assert!(!w.files.as_ref().unwrap().busy());
        assert!(w.control_file_job.is_none());
    ...
    #[test]
    fn rename_prompt_fences_cancel_repeat_and_native_submission() {
        for profile in [Profile::Windows, Profile::Emacs] {
            let dir = DialogDirectory::new();
            std::fs::write(dir.path("old"), b"disk").unwrap();
            let (mut w, _peer) = file_dialog_fixture();
            configure(&mut w, 800, 600);
            w.ui.dispatch(Event::Profile(profile)).unwrap();
            w.files.as_mut().unwrap().open(dir.0.clone()).unwrap();
            finish_file(&mut w);
            w.chord("R", true).unwrap();
            assert!(w.prompt.is_none());
            w.chord("R", false).unwrap();
            let id = w.last_dialog_id;
            w.path_chord("n", false);
            ...
            .unwrap();
            assert_eq!(std::fs::read(dir.path("old")).unwrap(), b"disk");
            w.chord("R", false).unwrap();
            assert_ne!(id, w.last_dialog_id);
            w.path_chord("n", false);
            w.path_chord("Return", true);
            ...
            assert!(w
                ...
                .fields()
                .unwrap()
                .contains("job=1,rename,2,0,0,complete,-"));
    ...
    fn remote_close_save_targets_unfocused_dialog_and_writes_before_closing() {
        ...
        let device = w.client.keyboard().unwrap();
        w.event(message(device, 2, &[99, SURFACE])).unwrap();
        assert!(!w.client.input().focused && !w.close_answer_visible());
    ...
    fn native_control_edits_match_both_key_profiles_and_pixels() {
        ...
        for profile in [Profile::Windows, Profile::Emacs] {
            ...
            let device = local.client.keyboard().unwrap();
    ...
    fn remote_selection_cancels_paste_and_repeat_only_after_success() {
        ...
        assert!(w.client.repeat(w.clock + 20).unwrap().is_some());
        ...
        assert!(w.client.repeat(w.clock + 100).unwrap().is_none());
    }
    ...
    #[test]
    fn path_completion_extends_lists_cycles_descends_and_submits_without_editing() {
        for profile in [Profile::Windows, Profile::Emacs] {
            let directory = DialogDirectory::new();
            std::fs::create_dir(directory.path("folder")).unwrap();
            std::fs::write(directory.path("folder/apple"), b"one").unwrap();
            std::fs::write(directory.path("folder/apricot"), b"two").unwrap();
            let (mut w, peer) = file_dialog_fixture();
            w.ui.dispatch(Event::Profile(profile)).unwrap();
            configure(&mut w, 800, 600);
            assert!(w.file_request("open", 1, 0));
            w.prompt.as_mut().unwrap().text = directory.path("fo").to_str().unwrap().into();
            let before = format!("{:?}", w.ui.editor());
            w.chord("Tab", true).unwrap();
            w.chord("Down", false).unwrap();
            w.chord("Up", false).unwrap();
            assert!(w.path_completion.is_none());
            w.chord("Tab", false).unwrap();
            finish_completion(&mut w);
            assert!(w.prompt.as_ref().unwrap().text.ends_with("folder/"));
            w.chord("Tab", false).unwrap();
            finish_completion(&mut w);
            assert!(w.prompt.as_ref().unwrap().text.ends_with("folder/ap"));
            let snapshot = prompt_snapshot(&mut w);
            assert!(snapshot.contains("completion=ready\tcompletion-count=2\t"));
            assert_eq!(snapshot.matches("completion-item=").count(), 2);
            let notice = w.path_notice().unwrap();
            assert!(notice.contains("apple") && notice.contains("apricot"));
            assert!(notice.lines().count() <= 6);
            assert!(notice.lines().all(|line| line.chars().count() <= 72));
            w.chord("S-Tab", false).unwrap();
            assert!(w.prompt.as_ref().unwrap().text.ends_with("apricot"));
            w.chord("Down", false).unwrap();
            assert!(w.prompt.as_ref().unwrap().text.ends_with("apple"));
            w.chord("Up", false).unwrap();
            assert!(w.prompt.as_ref().unwrap().text.ends_with("apricot"));
            assert_eq!(format!("{:?}", w.ui.editor()), before);
            w.chord("Return", false).unwrap();
    ...
    fn completion_results_cannot_revive_edited_cancelled_replaced_or_stale_prompts() {
        ...
        for change in ["edit-away-back", "cancel", "replace", "stale"] {
            let (mut w, _peer) = file_dialog_fixture();
            assert!(w.file_request("open", 1, 0));
            w.prompt.as_mut().unwrap().text = prefix.clone();
            w.prompt.as_mut().unwrap().completion = State::Pending(11);
            match change {
                "edit-away-back" => {
                    w.path_chord("x", false);
                    w.path_chord("Backspace", false);
                }
                "cancel" => w.path_chord("Escape", false),
                "replace" => {
                    w.path_chord("Escape", false);
                    assert!(w.file_request("open", 1, 0));
                    w.prompt.as_mut().unwrap().text = prefix.clone();
    ...
    #[test]
    fn completion_errors_counter_exhaustion_and_narrow_notice_preserve_entry() {
        let (mut w, _peer) = file_dialog_fixture();
        assert!(w.file_request("save-as", 1, 0));
        w.prompt.as_mut().unwrap().text = "literal".into();
        w.last_completion = u64::MAX;
        w.path_chord("Tab", false);
    ...
    fn window_close_cancel_preserves_discard_approvals_as_dirty_tabs() {
        ...
        w.client.input_mut().synchronized = true;
        w.chord("C-d", false).unwrap();
        assert!(!w.client.closed());
        assert_eq!(
            w.closing
    ...
    fn file_close_never_confirms_a_clipped_question_and_survives_input_loss() {
        ...
        assert!(w.closing.is_some() && w.client.keyboard().is_none());
    ...
    fn conflict_captions_fit_and_unready_or_stale_questions_never_confirm() {
        ...
        w.chord("C-r", false).unwrap();
        assert!(!w.conflict.as_ref().unwrap().needs_discard());
        w.client.input_mut().focused = true;
        let seat = w.client.seat().unwrap();
        w.event(message(seat, 0, &[1])).unwrap();
        caption(&w);
        w.chord("C-r", false).unwrap();
        ...
        send_map(&mut w, &peer, device, &map_file());
        focus(&mut w, device);
        let map = w.client.input_mut().map.take();
        caption(&w);
        w.chord("C-r", false).unwrap();
        assert!(!w.conflict.as_ref().unwrap().needs_discard());
        w.client.input_mut().map = map;
        w.ui.dispatch(Event::Resize {
            width: 208,
            height: 480,
            scale: 1,
    ...
    fn file_window_requires_a_v5_seat_and_uses_its_own_title() {
        for version in [None, Some(4), Some(5)] {
            ...
            if version != Some(5) {
                ...
                assert!(w.client.seat().is_none());
                assert!(drain(&peer).0.iter().all(|m| m.object != TOPLEVEL));
            } else {
                result.unwrap();
                assert!(w.client.seat().is_some());
    ...
    fn open_completion_never_repeats_into_the_new_active_tab() {
        for already_open in [false, true] {
            ...
            w.files.as_mut().unwrap().open(path).unwrap();
            w.client.input_mut().timing(1000, 0, w.clock).unwrap();
    ...
    fn replacement_map_and_capability_loss_cancel_input_without_losing_text() {
        ...
        assert!(w.client.input().map.is_none());
        assert!(!w.ui.focused());
    ...
    fn seat_removal_drains_retired_events_and_keeps_scratch_text() {
        ...
        assert!(w.client.seat().is_none() && w.client.keyboard().is_none() && !w.client.closed());
        assert_eq!(
            drain(&peer).0,
            [
                message(device, 0, &[]),
        ...
        w.event(message(seat, 0, &[2])).unwrap();
        send_map(&mut w, &peer, device, &map_file());
        assert!(w.client.keyboard().is_none() && w.client.input().map.is_none());
        assert_eq!(w.ui.editor().document(tab).unwrap().text(), text);
        w.event(message(DISPLAY, 1, &[device])).unwrap();
    ...
    fn initial_constructors_fill_reserved_ids_before_dynamic_seat_and_clipboard() {
        ...
        let manager = requests
            .iter()
            .filter(|request| request.object == REGISTRY)
            .find_map(|request| {
                let mut payload = Cursor::new(&request.payload);
                payload.u32().unwrap();
                (payload.string().unwrap() == "wl_data_device_manager").then(|| {
                    payload.u32().unwrap();
                    payload.u32().unwrap()
                })
            })
