...
- **Before rungs 25–26 there was no sound of any kind** — not "no
  PipeWire", no ALSA and no `/dev/snd`. That was a statement about the
  shipped kernel, not a non-goal. The audio pins below and `td-audio` are
  now landed; §K specifies the path from those devices to a jailed app.
  An earlier draft said "audio is a non-goal" ten lines above its own
  `CONFIG_SND*` pins, which was left over from before §K existed.
...
EXPERT-gated and already on. The namespace, resource and audio blocks
have all landed:
...
exited and released all it held. Both processes must have occupied a dedicated
application cgroup, and the marker is withheld until the production detached
watcher removes that leaf. Failure disables application launch with a named
diagnostic; it never silently selects a weaker sandbox.

**LANDED, in staged halves, and the split is worth reading before relying on
it.** The pins above are in
...
| `td-portal` | `/bin/td-portal` | none | surface **#12** | the activated desktop portal; the first static process serves Settings without a compositor connection (§E) |
...
`payload_inputs` entry, one typed `td-recipe-output` lock row, and one
direct canonical child of the active `/td/store`. The compiler currently
has exactly three reviewed runtime-major environment policies:
`empty-runtime`, `static-runtime`, and `freedesktop-platform-25-08`;
another runtime is refused until its policy is added deliberately. The
parser accepts only the compiler's canonical bytes, including the full
`/td/store` runtime path.
...
§H Firefox oracle remains. The Settings slice and bounded Request object core
of `td-portal` are now present. Session-producing interfaces, consent UI, file
mediation and the other service-side interfaces remain separate platform
prerequisites. None of those obligations is discharged by the bus/compositor
list below.
...
- per-instance portal activation and Request/Session handle routing are
  ...
  restarting. Settings consumes activation in the image, and the service now
  exports bounded caller-owned Request objects before their replies. Session
  routing is ready at the broker and object-table layers, but no shipped portal
...
on missing broker substrate: the filter, names, match rules, admission, pid
answers, descriptor budget and portal routing are all per-caller now. A second
application with a bus name still needs reviewed service decisions and exact
grants from the remaining bullets; it must not weaken the assertion merely
because the broker mechanisms exist. The other applications — mail, news
and Claude — hold no name and did not (§D).
...
```
 ...
 5  apps on the dynamic runtime only: /bin /sbin /lib /lib64 -> exact
        symlinks into usr/, with or without a package library path.
        Static empty-runtime apps omit the four aliases.
...
**The caller's directory reaches the entry as a NAME, never as a
descriptor.** An operator who runs a launcher from a directory expects the
program to start there, and the rule above is exactly why that cannot be
done by carrying the caller's directory handle across: an inherited
directory fd is the escape step 0 exists to close. So stage 1 reads its
own directory as a path while it still stands in it — before the unshare
and the mount plan make the answer unreadable — maps that path through the
grant that binds it, and passes the RESULT, a path in the jail's own
namespace, to stage 2 as one more word. The deepest containing grant
spells it, for the reason the next paragraph but one gives.

**Which side enforces what is worth stating exactly, because the
containment is stage 1's and not stage 2's.** Stage 1 derives the word
from a grant it has already resolved and holds itself to the wire format,
so a name it cannot carry — not UTF-8, or past the length bound — becomes
no directory rather than a launch stage 2 would refuse. Stage 2 checks the
SHAPE of the word, a bounded canonical absolute path that is not root, and
then tries to start the entry there. It does not verify that the path lies
under a grant target. So "only somewhere the policy already granted" rests
on stage 1's derivation, and stage 2's check is a structural backstop
rather than the boundary.

**The attempt is the check, and the fallback is real.** A readback could
not promise more than an attempt: `stat` succeeds without the search bit
that `chdir` needs, so a directory the entry's identity cannot enter would
pass every check and still fail, and the directory can be replaced between
the check and the use. So the entry is started in the directory and, if
that fails for any reason, started once more from stage 2's own `/`. A
spawn reports its failure before the entry ever execs, so the fallback
costs one fork and can never run the entry twice. A caller who stood
outside every grant, whose directory the mount plan did not place, or
whose directory the entry cannot enter is therefore not a failed launch:
the entry keeps `/`, which is where every launch started before this.

**Exactly one grant can ever answer.** Admitted grants never overlap: an
exact duplicate is merged, and any other pair overlapping in source or in
target is refused outright when the policy is resolved. So two sources can
never both be ancestors of the caller, and no target can nest inside
another — which is what rules out the case that would otherwise matter
here, a caller under an outer grant mapping onto an inner grant's mount
and starting the entry somewhere real and granted but not where the
operator stood. The mapping still takes the deepest match rather than the
first, so it stays correct for a grant set built directly rather than
resolved.

**One residual**, and it predates this: stage 2 inherits stage 1's
directory until `enter_mount_plan` sets `/`. No untrusted code runs in
that window, and it is why the two cleanup helpers name their `/`
explicitly.
...
the namespace, descriptor, mount and capability-bridge calls; rung 10 added
`wait4(2)` with final capability removal and its reparented-orphan oracle;
rung 11 added `seccomp(2)` plus no-new-privileges operations, the exact
compiled policy and its interpreter, build-host, and target-kernel oracles.
The quoted block mirrors the resulting implemented roster in `UNSAFE.md`;
`transition.rs` is the sole caller.
...
> and `ioctl(2)` with exactly THREE value-pinned requests:
> `SIOCGIFFLAGS`=0x8913 and `SIOCSIFFLAGS`=0x8914 use a pinned 40-byte
> `ifreq` with interface name pinned to `lo`, while `TIOCSCTTY`=0x540e uses
> scalar argument zero only after the fresh terminal's identity is proved —
> reached only from `transition.rs`,
...
one uniform read-only Settings service and an intentionally denied Background
Request. No consent or compositor-mediated interface and no Session-producing
interface is exported, so these services do not imply the rest of §E. Second,
an instance's
...
connect, and the broker would admit it as a peer that sees and addresses only
the portal and itself — but the two surfaces it could once reach, the shared
descriptor budget and its own init-namespace pid, are the closed gaps above.
What can be machine-checked today is narrower: the system recipe asserts that
exactly one shipped application's permission file carries a `[Session Bus
Policy]` entry, and that every shipped application has an evidence unit; a
second bus-holding entry, or an application without its oracle, breaks the
build. The first message routed between two applications' peers is an oracle
still to write.
...
directed signal observable; further applications now ship, and a second
bus-holding application, with traffic routed between two peers, remains a
...
Properties calls name the concrete portal interface; the empty-interface
shortcut is refused now that both Settings and Background publish a `version`
property and the answer would be ambiguous. `Peer.Ping` remains
path-independent as required by the standard interface.
...
synchronous `Settings.ReadAll` through td-busd, and byte-compares both replies
...
exported *before* the method reply, closing the subscription race. `Close` is
owner-only and completes a Request without a `Response`. Responses are 0
success, 1 cancelled, and 2 other.

This Request core is now landed. One service-wide table admits at most 64
combined handles and at most 16 per caller; tokens are 1–64 ASCII
alphanumeric/underscore bytes, td-busd's exact `:1.N` sender shape maps
injectively, active paths cannot collide, and a filtered `NameOwnerChanged`
removes every object owned by a departing caller when that best-effort broker
signal is delivered. Missing and foreign handles are indistinguishable to
`Close`, property and introspection calls; standard `Peer.Ping` is
path-independent and reveals neither. The
`org.freedesktop.portal.Background`
version 1 is its first honest consumer: because td has no persistent
background-service design, `RequestBackground` exports the caller's Request,
replies with that handle, then directs response 2 with exact false
`background` and `autostart` results and retires the object. `SetStatus` is a
version-2 method and is not advertised.

Sessions use `…/session/1_42/<token>` with owner-only `Close` and
portal-directed `Closed`. A caller's own `Close` receives only its method
reply; the portal emits `Closed` when it ends a session. The same bounded
table, dispatch, version property, introspection, departure cleanup and
unit/selftest coverage are landed, but no public portal creates a Session yet.
It is therefore staged machinery rather than a claimed live interface. The
next Session-producing interface must add a live export-before-reply,
portal-close, client-close and disconnect oracle before claiming it works.
Because td-busd may drop an ownership signal to a stalled peer, that landing
must also add a reliable owner-liveness reconciliation path; delivered
`NameOwnerChanged` is sufficient for today's same-dispatch Background denial,
not for a persistent Session lifecycle.
...
compatibility subset validates at most 32 filters, 32 rules per filter, and
the existing bounded label and pattern fields. It admits Firefox's native
Open File list only when `current_filter` exactly selects a member carrying
one `*` or `*.*` glob. That selected all-files filter is shown in a quoted
`APP FILTER` field after fixed status and returned with a successful response.
Other list members are validated compatibility metadata: the v1 chooser
neither offers them nor lets them constrain selection. A missing or mismatched
current filter for a multi-filter list, or a selected richer glob or MIME
filter, remains explicitly `NotSupported` rather than being silently
substituted.
...
`td-portal selftest`.
The system-image oracle now focuses Firefox with physical pointer input, sends
its native `Control+O` Open File command, requires the broker-authenticated
request's first modal frame, captures and validates its centred pixels through
QMP, selects the already authenticated download, and makes Firefox validate
the resulting exact `file:` document. No person inspects the screen.
...
- **`/etc/localtime`** — **read-only bind of the runtime's own compiled
  ...
  **Installation seeds the zone for applications and the native clock.**
  `td-install volume --timezone IANA-ID` validates the complete offline
  catalog before destination access and seeds `/var/lib/td/timezone`.
  The image's optional persistent `/etc/timezone` symlink points there;
  ...
  Invalid settings show `CLOCK ?`; absence stays UTC. The bounds and
  session snapshot are specified in `td-compositor/DESIGN.md`.
...
- **Audio: `td-audio`'s runtime, and no device.** This entry said
  ...
  read-only service runtime behind `/run/flatpak/pulse/native`, supplies
  the exact `PULSE_SERVER` and no-autospawn/no-shm client configuration,
  and never exposes `/dev/snd`. Mounting the directory rather than one
  socket inode lets a later connection from the same jail resolve the
  replacement after a daemon restart. The daemon alone receives playback
  nodes — direct ALSA would
  bypass per-app mediation, which is the reason it was refused when there
  was no sound at all and remains the reason now.
...
5. **Portal round-trips.** The landed Settings slice has pure dispatch tests
   ...
   filtering, invalid calls, and the closed settings file. Request tests pin
   ...
   drives Firefox's native Open File command, matches the portal's
   presented-frame dimensions and checksum to the captured modal pixels,
   accepts the selected file physically, and validates the exact resulting
   `file:` URL, MIME type and bytes in Firefox content.
...
6. **The offline QEMU oracle — the centrepiece.** Its synthetic first slice
   ...
   isolation or audio.
   The earlier plan's direct in-guest `mount(2) == EPERM` subprobe is no longer
   deferred. The dedicated outer-filter helper issues the real call on the
   target kernel, requires `EPERM`, and the bounded audit interval attributes
   that denial to the helper before its same-pid Firefox exec. The capability
   readbacks and standard-filter interpreter remain the structural checks. The
   landed
...
7. **keyboard, pointer, cursor, wheel, and a right-click menu opening and
   ...
   input frames. Tablet and PS/2 acknowledgements do not order their separate
   guest devices. The host therefore sends the first tablet position, waits
   for Firefox's compositor-observed cursor, sends the second in-window
   position, then waits for a bounded content probe to require a trusted
   motion event targeting the test input in that second position's fixed
   surface-local X band, document focus, and the still-focused test input.
   The proof-only transparent input spans the content viewport vertically, so
   target identity proves the point is web content without guessing Firefox's
   internal chrome offset. The host then
   physically clicks that already-attested input point to establish the
   compositor's keyboard focus before crossing from tablet to PS/2 input.
   Only that ordered click admits the key, wheel, and right-click frames. Each
   post-injection Marionette script waits at most
   twenty seconds for its event state within the probe's enclosing
   sixty-second deadline. Under TCG the guest can observe bounded repeats even
   though the host sends one acknowledged key-down/key-up command, so Firefox
   accepts one through four `x` characters and refuses zero, another
   character, or a fifth. It then requires pointer motion, input event,
...
9. **the Settings portal, Request lifecycle and FileChooser succeeding —
   ...
   an autotest-only preference. The portal validates Firefox's bounded native
   filter list and accepts its matching selected `All Files` current filter
   rather than falling back to a native GTK dialog.
   Firefox content first exposes a full-viewport focus control. One trusted
   primary click must focus the document and reach its one-shot refocus
   listeners. A fresh bounded, read-only Marionette session validates that
   ...
   already-delivered navigation result; no stale line or outstanding
...
12. `kill -KILL` of stage 1 reaping the whole instance — **LANDED**, as
    ...
    to test it. The probe now also creates a dedicated
    `td-jail-cleanup-<random>` application leaf through the production
    `ManagedCgroup` and detached `CgroupCleanup` path. The separate prefix
    preserves Firefox's one-active-instance rule. Stage 1 keeps the real
    watcher pipe across its descriptor sweep, attaches the blocked stage 2
    before releasing it, and passes the exact membership for stage 2 to read
    back. Before the kill, the driver twice requires stage 2 and its descendant
    to be the same live processes in that exact populated leaf. The watcher
    does not kill them: it waits for the liveness/PID-namespace teardown to
    empty the cgroup, then removes it. The marker is withheld until the driver
    observes that removal under a separate deadline. The source confinement
    regression pins the shared transition, attach-before-release,
    membership-readback, sole stage-1 kill, and final removal wait, while the
    every-boot target oracle is the kernel authority;
...
14. **unsupported portals unavailable — LANDED.** The same unprivileged live
    ...
    result.
Separately from those machine-checkable proof items, the landing review
reconciles the application-path unsafe rosters. The shipped sources and their
derived confinement inventories agree with all four entries: `td-jail` is
surface #9, `td-busd` is #10, `td-portal` is #12, and `td-audio` is #13. Their
confinement tests inventory the compiled source sets, scoped allowances,
syscall numbers, and each component's operation, flag, ancillary-data,
request, and production-caller bounds. The exact crate suites and strict
Clippy pass with those assertions; source review separately reconciles the raw
pointer provenance described by each entry. This adds no syscall or allowance
and therefore requires no roster amendment.
...
| 27c | **Firefox physical-input image proof — LANDED**; the first system boot uses a bounded private QMP controller, the already enumerated virtio tablet, and the PC machine's PS/2 keyboard, while staged Firefox content/chrome probes and a compositor-owned cursor marker independently attest the semantic results. Because host QMP acknowledgements do not order those separate guest devices, the host sends one tablet position, waits for Firefox's compositor-observed cursor, sends a second in-window position, and withholds the key until a bounded content stage observes a trusted motion event targeting the test input in that second position's fixed surface-local X band plus document and input focus. The proof-only transparent input spans the content viewport vertically, so its target identity proves the point is web content without guessing Firefox's internal chrome offset. It then physically clicks that attested input point to establish compositor keyboard focus before typing. The remaining handshake proves typing, scrolling, native right-click menu opening, outside-click dismissal, and a painted Firefox cursor without sleeps or manual observation. The test page retains the authenticated magenta/lime framebuffer sentinel and adds only that fixed input, scroll extent, and cursor style | the supported Firefox path accepts real emulated input end to end; rung 27d extends the same handshake across clipboard paste |
...
| 29 | **native clock in local time — LANDED**: the bar snapshots rung 12k's saved name and source-built TZif v2/v3 data, including future DST recurrence. Absence stays UTC; invalid or unsupported data displays `CLOCK ?`. `td-compositor/DESIGN.md` owns bounds, session lifetime and tests | the bar shows local civil time with the actual UTC offset |
...
| 35 | **news in a td-ui window — LANDED** (§W.8, "Reworked"): td-ui gains the cell screen, a grid of styled cells presented as a plain toplevel that polls the program's handler each turn, and td-news's views draw on it whole; its terminal layer and UNSAFE.md §18 surface are gone, the crate forbids `unsafe`, the recipe is a static PIE Cargo build over the toolkit, the `news` package drops `devices=tty`, and the `[news]` unit launches it direct and is ready when the compositor's layout names its toplevel. Mail follows in its own increment. Reworked again (§W.8, "Reworked again"): for news the toolkit's lists and td-editor's read-only document pane in the widget window replace the cell screen, since deleted (`td-ui/DESIGN.md`, increment 13), and the recipe stages `td-editor` beside `td-ui` | one of the two source-built applications no longer needs a terminal, a pty grant or td-term to exist |
| 36 | **mail in a td-ui window — LANDED** (§W.8, "Reworked"): td-mail's views keep their cursor-and-attribute writer over the window's cell screen, so they move untouched; its terminal layer and UNSAFE.md §17 are gone and the crate forbids `unsafe`; the recipe is a static PIE Cargo build over the toolkit, the `mail` package drops `devices=tty`, and the `[mail]` unit launches it direct with the anchored layout probe. The direct-rustc local-source ladder keeps one user, td-install-qemu-test. Reworked again (§W.8, "Reworked again"): for mail too the toolkit's lists and td-editor's read-only document pane in the widget window replace the cell screen, since deleted, and the recipe stages `td-editor` beside `td-ui`; composing is in an editable pane in the same window (§W.8, "Composing in place"), so the package ships no editor | neither source-built application needs a terminal, a pty grant or td-term to exist; Claude is the terminal set, and §W.7's relaunch is a window request |
...
discovers the host socket and presents it read-only at the fixed path
...
includes **`enable-shm=no`**. td keeps those exact client-visible paths
and bytes, adds `autospawn=no`, and makes `native` a fixed alias into a
read-only bind of the service runtime. The directory bind is deliberate:
unlike a bind of today's socket inode, it exposes the replacement pathname
after td-svc restarts the daemon. The highest private ancestor is a read-only
mountpoint, making the generated configuration and both private directories
immutable: the application cannot rename an ancestor, recreate the fixed
path and replace the policy.
Before creating the jail, td-jail also completes a bounded connection to the
resolved socket. A dead daemon's leftover socket inode is therefore not
authority and cannot admit a permanently silent application.
...
`[i32; 3]`. The landed multicall provides the server, deterministic ALSA
and WAV tone paths, WAV verification, device enumeration and a live-socket
probe. `status` and `volume` remain future ordinary Pulse clients of the
daemon's own socket, so policy tooling will cost no second protocol.
...
*The dedicated account, devices and service are landed.*
...
It recognizes only `/dev/snd/pcmC<card>D<device>p`, caps the scan at 64
entries, and assigns those playback nodes mode 0600 to `audio`; capture,
control and unknown nodes retain their existing ownership. The locked
account remains refused by `td-login exec-as`. A separate exact
`exec-service-as` path admits only the service-only shadow marker and is
the daemon unit's credential transition, rather than `su`, a passwordless
account, or a general `User=` directive in td-svc. The unit requires seat
setup. On every start, `td-seatd exec-audio` repeats the bounded playback
scan and assignment before literally executing the service credential
transition and `td-audio serve`; this covers sound nodes that enumerate
after the seat oneshot without giving the daemon root. Its readiness probe
connects to that running instance's socket under the same service identity.
With no non-loopback playback PCM, device selection fails and the service
remains in supervised restart rather than exposing a silent fake sink;
because Firefox's audio grant is mandatory, its launch then fails closed
until usable hardware appears. Supporting a physical machine still requires
the explicit device and firmware matrix below.
...
Kernel and QEMU: `intel-hda` with playback-only `hda-output` as the first target
...
1. `builder/src/affected.rs` — **LANDED**: `DEPENDENCY_FREE_LOCKS` and
   ...
   preflight's declares `gate-test-args` beside `test-args`. The host tools
   td-review and td-vm use this split for their host integration tests;
   their two legs are pinned apart by a test.
...
td-mail executed a plain-word editor command directly and kept `sh -c`
...
**Reworked: composing in place.** td-mail no longer runs an editor
child: it composes in td-editor's document view embedded in its own
...
### W.8 td-mail and td-news: the two applications as td code
...
**Reworked: td-news draws in a td-ui window.** This paragraph is the
record of an increment since superseded: both programs draw in td-ui's
widget window now ("Reworked again", below) and the cell screen it
describes is deleted (`td-ui/DESIGN.md`, increment 13). The terminal was
the shape the two programs arrived in, not a decision about how td shows
them, and it cost each a raw-terminal layer with an `unsafe` surface
(UNSAFE.md §17 and §18), the `devices=tty` grant, a td-term window to
live in and the relaunch problem §W.7 describes. td-ui's cell screen
keeps the shape the views draw in, rows and columns of styled cells, and
presents it as a plain Wayland toplevel, so the views move whole.
td-news goes first: its views paint a `Screen`, its keys arrive
translated from the keyboard's chords, a click is the cell under the
pointer and a wheel frame is rows of travel; its backend thread's
channel is read by the window's poll each turn. Two things change for
the reader with that: the wheel now scrolls the article view, which its
help line always promised and the terminal's mouse-off there prevented;
and the article view's text selection, which that mouse-off existed for,
is gone, since the toolkit's screen has no selection and no clipboard.
Copying article text is a toolkit increment to come, not a reader
change. The terminal layer and `term_sys.rs` are gone, the crate forbids
`unsafe` outright and UNSAFE.md §18 is retired. The crate depends on
td-ui by the one sibling spelling the gate admits, so its recipe is
`Recipe::rust` with `td-ui` and `td-compositor` staged beside it, as
td-taskmgr's is, a static PIE rather than direct rustc's fixed-address
executable; rust-userland-auto-test holds it to that shape. The `news`
package keeps `sockets=wayland;fetch` and drops the terminal grant; the
`[news]` unit launches it `direct` as Firefox is, with no `terminal`
ordering, and is ready when the compositor's layout report names its
toplevel by app id in a record anchored at its every field, so a title
cannot forge one, polled to the unit's timeout as td-term's socket probe
was. td-firstboot's provisioning, the fetch service, the boot marker and
the evidence unit are unchanged. td-mail followed in its own increment
by the same route: its views keep their cursor-and-attribute writer,
`Terminal`, now a render's writer over the window's `Screen` with the
theme's colours as styles and the HTML rendering's SGR sequences read
into cell attributes, so the view files are untouched but for the
mailbox list's account switch, which reopens a lone account, and the
hint and help that say so; its keys, clicks and wheel travel arrive
translated as td-news's do, into the client's own key type; the session
polls the backend's channel, the pending actions and the idle-sync clock
each turn; the draft is retained and the editor spawned as before, a
path §W.5 still owns. The window opens before the account connects, the
session's one connector thread making the connection behind it, the
backend holding every command until the connection is decided and the
mailbox list loading until then, so the credential and discovery round
trips never hold the toplevel the unit's readiness waits for and nothing
is read from the cache that the connection supersedes. The same terminal
text selection goes with it. Its terminal layer and UNSAFE.md §17 are
...
moved, §W.7's relaunch problem is a launcher request for a window rather
than a pty, and §D's terminal set is Claude alone.

**Reworked again: td-news shows its lists and reads an article in
td-editor's document view.** The cell screen kept the terminal's shape,
a grid of styled cells, and that is the wrong abstraction for a reader:
the feeds and the articles are lists, and an article is a text to read.
td-news now draws in td-ui's widget window (`td-ui/DESIGN.md`, "Widget
window"): every view is an action bar whose labels are the view's keys,
a body and the status row, the body a toolkit `List` for the feeds, for
the articles under a search field and for an article's links, or
td-editor's document pane (`td-editor/DESIGN.md`, "Embedding the
document view"), read-only, for the article, a window of the log and the
...
lists and reading, below, then composing in an editable pane in place of
the `$EDITOR` child. The cell screen and its window are deleted
(`td-ui/DESIGN.md`, increment 13).
...
entry while it reads a folder name or a search, a body that is a list of
rows or a text, and the status row. The session lays the scene out in
the widget window each frame, a toolkit `List` for the mailboxes, the
messages, a thread's messages, a move-to-folder picker and a message's
links, and td-editor's read-only document pane for a message, a thread
read whole, the help and the retention and rules previews, reloaded only
when the text it shows or the pane's columns change. The list keeps its
selection in view, a press on a row is the row's key to the view, the
...
window are deleted (`td-ui/DESIGN.md`, increment 13).

**Composing in place: td-mail edits a draft in td-editor's document
view.** `c`, a reply or a forward retains the draft as before, the
`.eml` file under the state directory with its attachment sidecar, and
then opens it in the pane, editable, its paragraphs filled as they are
typed, in place of the `$EDITOR` child. While the draft is edited every
chord is the pane's, in td-editor's default profile: Ctrl-S writes the
pane's text over the retained file and marks the document saved at that
state; Ctrl-W pops a saved draft and asks about an unsaved one, where
`y` saves and pops, `n` pops keeping the file as it was last saved and
Escape returns to the draft; the bar's Save and Close labels are the
same requests, and while the question is up no chord or press reaches
the draft. A save is whole or not at all: a private sibling written and
renamed over the retained file, so a failed write leaves the draft as it
was and a symlink at the path is replaced, not followed; while the
composer holds a draft the file is its own. The window's close with an
unsaved draft puts the same question instead of closing (the widget
window now lets its handler keep the window on a close request,
`td-ui/DESIGN.md`), so nothing typed is lost to the close and a failed
save is seen; nothing here ever deletes a draft. A draft past the pane's
...
in every case is flatpak's: **bind the host's authority into the jail.**
...
| `td-audio` | absent. Bind the host's read-only **PulseAudio-protocol runtime** — `pipewire-pulse`'s where the host runs PipeWire — behind §K.2's fixed `native` alias. Not `pipewire-0`: §K.1 keeps those apart and they are different protocols |
