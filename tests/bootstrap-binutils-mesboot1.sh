#!/bin/sh
# tests/bootstrap-binutils-mesboot1.sh — source-bootstrap BRICK 5 (gcc toolchain): GNU Binutils 2.20.1a
# REBUILT by gcc-mesboot0 against glibc — guix's binutils-mesboot1. binutils-mesboot0 (brick rung 376)
# was built by tcc against the mes libc; now that td has a real gcc (gcc-mesboot0) and a real C library
# (glibc-mesboot0), it rebuilds binutils so as/ld/ar are gcc-built and glibc-linked — the assembler and
# linker that the next compiler (gcc-mesboot1, 4.6.4) is built with.
#
# From the 229-byte seed, td builds the whole chain through gcc-mesboot0 (bricks 0-4 + make/patch/
# binutils/gcc/glibc/gcc-mesboot0 rungs), then rebuilds binutils with a PLAIN configure (guix's
# binutils-mesboot1 drops binutils-mesboot0's tcc/MES_BOOTSTRAP overrides): CC=<gcc-mesboot0>, the real
# binutils-mesboot0 ar/ranlib, glibc-mesboot0 as libc, the host-produced kernel headers. The boot patch
# is still applied — its MES_BOOTSTRAP #ifdef branches now compile the real-glibc side. Plus the
# make-in-sandbox fixes (SHELL var, cleared MAKEFLAGS jobserver) and the /bin/sh-shebang rewrite.
#
...
# (td-feed warm kernel-headers i386, run in check.sh's prelude — like warm-tsgo); this gate CONSUMES them.
#
# i686, static, serial. mes/nyacc/tcc/make/patch/binutils/gcc/glibc/linux are td-fetched; the 4 guix
# boot patches are vendored source data — as/ld/ar are compiled from binutils source + the patch (no
# guix-built bytes; the [no-guix] leg verifies).
#
# Legs (DURABLE):
#   [pinned-input] 9 tarballs (…/binutils/gcc/glibc/linux) + the 4 vendored boot patches match sha256.
#   [no-guix]      built on a curated PATH with gcc/g++/cc/guile/guix DENIED; no /gnu/store in as/ld/ar.
#   [behavioral]   as/ld report 2.20; gcc-mesboot0 + the NEW as+ld assemble+link+RUN a static C
#                  program → 42 — a gcc-built, glibc-linked binutils, not the tcc/mes-libc one.
#   [repro]        two independent binutils-mesboot1 builds (same dir) yield a byte-identical as + ld.
set -eu
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$cpath"`"' EXIT INT TERM

AS="$B2/out/bin/as"; LD="$B2/out/bin/ld"; AR="$B2/out/bin/ar"

# --- [no-guix] -------------------------------------------------------------------------------
for art in "$AS" "$LD" "$AR"; do test -x "$art" || fail "binutils-mesboot1 missing $art"; if grep -q -a '/gnu/store' "$art"; then fail "$art contains /gnu/store bytes"; fi; done
echo "   [no-guix] seed → … → glibc → gcc-mesboot0 → binutils-mesboot1 built with no gcc/guile/guix on PATH; no /gnu/store in as/ld/ar"

# --- [behavioral] as/ld report 2.20 and the NEW as+ld assemble+link+RUN a static C program → 42 -----
av=`env -i "$AS" --version 2>"$B2/as.err" | head -1` || { tail -3 "$B2/as.err" >&2; fail "binutils-mesboot1 as did not run"; }
echo "$av" | grep -q '2\.20' || fail "binutils-mesboot1 as --version gave [$av], want 2.20.x"
lv=`env -i "$LD" --version 2>"$B2/ld.err" | head -1` || { tail -3 "$B2/ld.err" >&2; fail "binutils-mesboot1 ld did not run"; }
echo "$lv" | grep -q '2\.20' || fail "binutils-mesboot1 ld --version gave [$lv], want 2.20.x"
GCC2="$G2/out/bin/gcc"; G2DIR="$G2/out/lib/gcc-lib/i686-unknown-linux-gnu/2.95.3"
wd=`mktemp -d`; printf '#include <stdio.h>\nint main(){printf("hi");return 42;}\n' > "$wd/t.c"
# put binutils-mesboot1's as/ld FIRST so gcc-mesboot0 uses the NEW assembler+linker for the round-trip;
# crt via LIBRARY_PATH (no -B), pure kernel UAPI ($B2/kh) + glibc on the include path.
( cd "$wd" && env PATH="$B2/out/bin:$cpath" C_INCLUDE_PATH="$GLD/out/include:$B2/kh" LIBRARY_PATH="$GLD/out/lib:$G2DIR" \
    "$GCC2" -static -o t t.c ) >"$wd/cc.log" 2>&1 \
  || { tail -10 "$wd/cc.log" >&2; rm -rf "$wd"; fail "the new as+ld could not assemble+link a C program"; }
head -c20 "$wd/t" | od -An -tx1 | grep -q '7f 45 4c 46 01' || { rm -rf "$wd"; fail "did not emit a 32-bit ELF"; }
set +e; ( cd "$wd" && env -i ./t ); rc=$?; set -e
rm -rf "$wd"
test "$rc" = 42 || fail "the program assembled+linked by binutils-mesboot1 returned $rc, want 42"
echo "   [behavioral] as→'$av'; ld→'$lv'; the new as+ld assembled+linked+ran a static C program → 42"

# --- [repro] a second independent binutils-mesboot1 build (same dir) is byte-identical -------------
a1=`sha "$AS"`; l1=`sha "$LD"`
build_binutils_mesboot1 "$cpath" "$G2" "$BD" "$GLD" "$MK" "$PD" "$B2" || fail "the second binutils-mesboot1 build did not run"
test "$a1" = "`sha "$AS"`" || fail "binutils-mesboot1 as is NOT reproducible — r1=$a1 r2=`sha "$AS"`"
test "$l1" = "`sha "$LD"`" || fail "binutils-mesboot1 ld is NOT reproducible — r1=$l1 r2=`sha "$LD"`"
echo "   [repro] two independent binutils-mesboot1 builds produce a byte-identical as + ld (reproducible)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, td rebuilt GNU Binutils 2.20.1a with"
echo "      gcc-mesboot0 against glibc (guix's binutils-mesboot1): a gcc-built, glibc-linked as+ld that"
echo "      assemble+link+run C → 42; no gcc/guile/guix, no /gnu/store, reproducible. make-mesboot 3.82 next."
