#!/bin/sh
# tests/bootstrap-gcc-mesboot.sh — source-bootstrap BRICK 5 (gcc toolchain), the FINAL mesboot gcc:
# GCC 4.9.4 — guix's gcc-mesboot. After glibc-mesboot (2.16.0, the modern nptl C library), the
# gcc-mesboot1 (GCC 4.6.4, C++) toolchain + binutils-mesboot build GCC 4.9.4 against glibc 2.16.0. This
# is the last gcc in the Mes bootstrap chain — capable enough to (re)build the modern toolchain.
#
# From the 229-byte seed, td builds the whole chain (bricks 0-4 + make/patch/binutils/gcc/glibc rungs +
# gcc-mesboot0 + binutils-mesboot1 + make-mesboot + gcc-mesboot1 + binutils-mesboot + gawk-mesboot +
# glibc-mesboot 2.16.0), then builds GCC 4.9.4 from one tarball (no modular g++ download, no boot patch —
# guix deletes both phases), with gmp-4.3.2/mpfr-2.4.2/mpc-1.0.3 unpacked in-tree. CC=<gcc-mesboot1>.
#
# td builds GCC 4.9.4 STATIC (guix builds it --enable-shared via the gcc-mesboot1-wrapper's dynamic
# linker): td's glibc-mesboot is static-only (no libc.so), so a default/dynamic link fails (libgcc's
# unwinder references dl_iterate_phdr). The static divergence falls out as link-only flags that keep CC
# itself clean (so autoconf header tests aren't polluted by a -B "never used" warning, the binutils-
# mesboot1 lesson): LDFLAGS=-static -B<glibc>/lib for the host link tests, CC_FOR_BUILD="<gcc> -static"
# for the in-tree gmp/mpfr/mpc build tools (which compile AND run during the build), --disable-shared.
#
# i686, static, serial. all sources td-fetched; gcc-4.9.4 is built from the pristine upstream tarball
# (the 7 guix origin patches all touch DISABLED components — libsanitizer/libvtv/asan/arm/texi2pod — so
# the static c,c++ build needs none; the [no-guix] leg verifies no guix bytes land in the driver/cc1).
#
# Legs (DURABLE):
#   [pinned-input] chain tarballs + vendored boot patches + gcc-4.9.4 match sha256.
#   [no-guix]      built on a curated PATH with gcc/g++/cc/guile/guix DENIED; no /gnu/store in gcc/cc1.
#   [behavioral]   gcc-mesboot (4.9.4) compiles+links a C program AND a C++ program (libstdc++) against
#                  glibc 2.16.0 statically — both RUN → 42 — a modern C/C++ compiler from source.
#   [repro]        the gcc/cpp DRIVERS reproduce byte-for-byte AND `gcc -S` output is deterministic
#                  across two independent builds (cc1 carries a benign stabs stamp; the driver is durable).
set -eu
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BMB"`" "`dirname "$GAWKMB"`" "`dirname "$GOUT"`" "`dirname "$GMB"`" "`dirname "$cpath"`"' EXIT INT TERM

GCCM="$GMB/out"; GO="$GOUT/out"; GMBDIR="$GCCM/lib/gcc/i686-unknown-linux-gnu/4.9.4"
CC1=`ls "$GCCM"/libexec/gcc/i686-unknown-linux-gnu/4.9.4/cc1 2>/dev/null || true`

# --- [no-guix] -------------------------------------------------------------------------------
test -x "$GCCM/bin/gcc" -a -x "$GCCM/bin/g++" -a -x "$GCCM/bin/cpp" || fail "gcc-mesboot missing gcc/g++/cpp"
for b in "$GCCM/bin/gcc" "$GCCM/bin/g++" "$GCCM/bin/cpp" "$CC1"; do
  test -n "$b" -a -f "$b" || fail "gcc-mesboot output missing ($b)"
  if grep -q -a '/gnu/store' "$b"; then fail "$b contains /gnu/store bytes"; fi
done
echo "   [no-guix] seed → … → gcc-mesboot1 → glibc 2.16.0 → GCC 4.9.4 built with no gcc/g++/cc/guile/guix on PATH; no /gnu/store in gcc/g++/cpp/cc1"

# --- [behavioral] gcc-mesboot 4.9.4 compiles+links a C program AND a C++ (libstdc++) program → 42 -------
wd=`mktemp -d`
printf '#include <stdio.h>\nint main(){printf("hi");return 42;}\n' > "$wd/t.c"
( cd "$wd" && env -i PATH="$BMB/out/bin:$cpath" LIBRARY_PATH="$GO/lib:$GMBDIR" \
    "$GCCM/bin/gcc" -static -B"$GO/lib" -o tc t.c ) >"$wd/cc.log" 2>&1 \
  || { tail -12 "$wd/cc.log" >&2; rm -rf "$wd"; fail "gcc-mesboot could not link a C program against glibc 2.16.0"; }
head -c20 "$wd/tc" | od -An -tx1 | grep -q '7f 45 4c 46 01' || { rm -rf "$wd"; fail "gcc-mesboot C output not a 32-bit ELF"; }
set +e; ( cd "$wd" && env -i ./tc ); rcc=$?; set -e
test "$rcc" = 42 || { rm -rf "$wd"; fail "the gcc-mesboot C program returned $rcc, want 42"; }
printf '#include <string>\n#include <cstdio>\nint main(){std::string s="hi";printf("%%s",s.c_str());return 42;}\n' > "$wd/p.cc"
( cd "$wd" && env -i PATH="$BMB/out/bin:$cpath" LIBRARY_PATH="$GO/lib:$GMBDIR" \
    "$GCCM/bin/g++" -static -B"$GO/lib" -o tcpp p.cc ) >"$wd/cxx.log" 2>&1 \
  || { tail -15 "$wd/cxx.log" >&2; rm -rf "$wd"; fail "gcc-mesboot could not link a C++ (libstdc++) program against glibc 2.16.0"; }
set +e; ( cd "$wd" && env -i ./tcpp ); rcp=$?; set -e
rm -rf "$wd"
test "$rcp" = 42 || fail "the gcc-mesboot C++ program returned $rcp, want 42"
echo "   [behavioral] gcc-mesboot 4.9.4 compiled+linked a C program → 42 AND a C++ (libstdc++) program → 42 against the static glibc 2.16.0 — a modern C/C++ compiler from source"

# --- [repro] the gcc/cpp drivers byte-identical AND `gcc -S` output deterministic ---------------------
# cc1 is NOT byte-identical (a benign stabs stamp, same class as the gcc-core-mesboot1 rung); assert the
# DURABLE form: the gcc/cpp DRIVER binaries reproduce byte-for-byte AND the compiler's `-S` assembly
# output is deterministic across two independent builds.
g1=`sha "$GCCM/bin/gcc"`; p1=`sha "$GCCM/bin/cpp"`
rs=`mktemp -d`; printf '#include <stdio.h>\nint fib(int n){return n<2?n:fib(n-1)+fib(n-2);}\nint main(){return fib(9);}\n' > "$rs/r.c"
env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" "$GCCM/bin/gcc" -S -O2 -o "$rs/a.s" "$rs/r.c" || { rm -rf "$rs"; fail "gcc-mesboot could not emit -S assembly"; }
a1=`sha "$rs/a.s"`
build_gcc_mesboot "$cpath" "$GM1" "$BMB" "$GOUT" "$MM" "$PD" "$GMB" || { rm -rf "$rs"; fail "the second gcc-mesboot build did not run"; }
test "$g1" = "`sha "$GCCM/bin/gcc"`" -a "$p1" = "`sha "$GCCM/bin/cpp"`" || { rm -rf "$rs"; fail "gcc-mesboot gcc/cpp drivers are NOT reproducible"; }
env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" "$GCCM/bin/gcc" -S -O2 -o "$rs/b.s" "$rs/r.c" || { rm -rf "$rs"; fail "gcc-mesboot (rebuild) could not emit -S assembly"; }
test "$a1" = "`sha "$rs/b.s"`" || { rm -rf "$rs"; fail "gcc-mesboot `gcc -S` output is NOT deterministic"; }
rm -rf "$rs"
echo "   [repro] the gcc/cpp driver binaries are byte-identical AND \`gcc -S\` assembly output is deterministic across two independent builds (cc1 carries only a benign stabs stamp; the driver + output are durable)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, td built GCC 4.9.4 (guix's gcc-mesboot):"
echo "      the FINAL mesboot gcc, built by gcc-mesboot1 + binutils-mesboot against the static glibc"
echo "      2.16.0, that compiles+links a C program AND a C++ program → 42; no gcc/guile/guix, no"
echo "      /gnu/store, reproducible. The Mes full-source bootstrap now reaches a modern GCC."
