#!/bin/sh
# tests/bootstrap-gcc-mesboot1.sh — source-bootstrap BRICK 5 (gcc toolchain): GCC 4.6.4 with C AND C++ —
# guix's gcc-mesboot1. gcc-core-mesboot1 (#176) proved the C build; this adds the gcc-g++-4.6.4 front-end
# (overlaid into the gcc tree) + --enable-languages=c,c++, so the build also produces cc1plus + a static
# libstdc++. This is the C++ compiler the next gcc (gcc-mesboot 4.7.4, which is itself C++) requires.
#
# From the 229-byte seed, td builds the whole chain (bricks 0-4 + make/patch/binutils/gcc/glibc rungs +
# gcc-mesboot0 + binutils-mesboot1 + make-mesboot), then make-mesboot drives the GCC 4.6.4 c,c++ build:
# CC=<gcc-mesboot0>, as/ld/ar = binutils-mesboot1, libc = glibc-mesboot0, pure host-produced kernel UAPI
# headers, gmp/mpfr/mpc unpacked in-tree. td's glibc-mesboot0 is static-only, so (unlike guix's
# -dynamic-linker) td builds it STATIC: LDFLAGS="-static -B<glibc>/lib" (link-only). MAKEINFO=true skips
# the texinfo docs; cmp/diff are linked from the store (gcc 4.6.4's install regenerates tm.texi via
# move-if-change, which the loop sandbox's diffutils-free PATH otherwise can't run). CPLUS_INCLUDE_PATH
# mirrors C_INCLUDE_PATH so the C headers are found while libstdc++ is compiled (guix's setenv).
#
# KERNEL HEADERS: produced FROM the pinned linux-4.14.67 source on the HOST (td-feed warm kernel-headers i386,
# check.sh's prelude); this gate CONSUMES them (guix's %bootstrap-linux-libre-headers is a rejected blob).
#
# i686, static, serial. mes/…/gcc-4.6.4/gcc-g++-4.6.4/gmp/mpfr/mpc are td-fetched; the 5 guix boot patches
# are vendored source data — gcc/g++/cc1/cc1plus are compiled from source (no guix bytes; [no-guix] verifies).
#
# Legs (DURABLE):
#   [pinned-input] the chain's tarballs + 5 vendored boot patches + gcc-4.6.4/gcc-g++/gmp/mpfr/mpc match sha256.
#   [no-guix]      built on a curated PATH with gcc/g++/cc/guile/guix DENIED; no /gnu/store in gcc/g++.
#   [behavioral]   gcc AND g++ report 4.6.4; gcc compiles+runs a C program → 42 AND g++ compiles+runs a
#                  C++ program (templates+classes, static libstdc++) → 42.
#   [repro]        the gcc + g++ drivers are byte-identical across two builds AND both emit byte-identical
#                  assembly (intrinsic output-repro; cc1/cc1plus binaries carry benign build stamps).
set -eu
...
# --- [pinned-input] extras: make-3.82 + gcc-4.6.4 + gcc-g++-4.6.4 + gmp/mpfr/mpc + the gcc-4.6.4 patch -
...
echo "   [pinned-input] + make-3.82, gcc-4.6.4, gcc-g++-4.6.4, gmp/mpfr/mpc + the gcc-4.6.4 boot patch match their pins"
...
GC1=`mktemp -d`/gccmesboot1build; build_gcc_mesboot1 "$cpath" "$G2" "$B2" "$MM" "$GLD" "$PD" "$GC1" || fail "the toolchain did not build GCC 4.6.4 (c,c++)"
...
GCC1="$GC1/out/bin/gcc"; GXX="$GC1/out/bin/g++"; GC1DIR="$GC1/out/lib/gcc/i686-unknown-linux-gnu/4.6.4"

# --- [no-guix] -------------------------------------------------------------------------------
test -x "$GCC1" -a -x "$GXX" || fail "gcc-mesboot1 missing gcc or g++"
for art in "$GCC1" "$GXX"; do if grep -q -a '/gnu/store' "$art"; then fail "$art contains /gnu/store bytes"; fi; done
echo "   [no-guix] seed → … → gcc-mesboot0 → binutils-mesboot1 → make-mesboot → gcc-mesboot1 built with no gcc/guile/guix on PATH; no /gnu/store in gcc/g++"

# --- [behavioral] gcc + g++ report 4.6.4; C → 42 AND C++ → 42 --------------------------------------
gv=`env -i "$GCC1" --version 2>"$GC1/gcc.err" | head -1` || { tail -3 "$GC1/gcc.err" >&2; fail "gcc did not run"; }
xv=`env -i "$GXX" --version 2>"$GC1/gxx.err" | head -1` || { tail -3 "$GC1/gxx.err" >&2; fail "g++ did not run"; }
echo "$gv" | grep -q '4\.6\.4' || fail "gcc --version gave [$gv], want 4.6.4"
echo "$xv" | grep -q '4\.6\.4' || fail "g++ --version gave [$xv], want 4.6.4"
wd=`mktemp -d`
...
( cd "$wd" && env -i PATH="$B2/out/bin:$cpath" C_INCLUDE_PATH="$GLD/out/include" LIBRARY_PATH="$GLD/out/lib:$GC1DIR" \
    "$GCC1" -static -B"$GLD/out/lib" -o tc t.c ) >"$wd/cc.log" 2>&1 || { tail -12 "$wd/cc.log" >&2; rm -rf "$wd"; fail "gcc could not compile+link a C program"; }
set +e; ( cd "$wd" && env -i ./tc ); rcc=$?; set -e
test "$rcc" = 42 || { rm -rf "$wd"; fail "the gcc-built C program returned $rcc, want 42"; }
printf 'template<class T> T sq(T x){return x*x;}\nstruct B{int v;B(int n):v(n){}};\nint main(){B b(6);return sq(b.v)+6;}\n' > "$wd/t.cpp"
( cd "$wd" && env -i PATH="$B2/out/bin:$cpath" C_INCLUDE_PATH="$GLD/out/include" LIBRARY_PATH="$GLD/out/lib:$GC1DIR" \
    "$GXX" -static -B"$GLD/out/lib" -o tcpp t.cpp ) >"$wd/cxx.log" 2>&1 || { tail -15 "$wd/cxx.log" >&2; rm -rf "$wd"; fail "g++ could not compile+link a C++ program"; }
head -c20 "$wd/tcpp" | od -An -tx1 | grep -q '7f 45 4c 46 01' || { rm -rf "$wd"; fail "g++ did not emit a 32-bit ELF"; }
set +e; ( cd "$wd" && env -i ./tcpp ); rcx=$?; set -e
rm -rf "$wd"
test "$rcx" = 42 || fail "the g++-built C++ program returned $rcx, want 42"
echo "   [behavioral] gcc→'$gv', g++→'$xv'; gcc ran a C program → 42 AND g++ ran a C++ program (templates+classes, static libstdc++) → 42"

# --- [repro] gcc + g++ drivers byte-identical AND both emit identical assembly across a rebuild -------
gd1=`sha "$GCC1"`; xd1=`sha "$GXX"`
rs=`mktemp -d`; printf 'int f(int x){return x*x+1;}\n' > "$rs/r.c"; printf 'template<class T>T sq(T x){return x*x;}\nint g(int n){return sq(n)+n;}\n' > "$rs/r.cpp"
env -i PATH="$B2/out/bin:$cpath" "$GCC1" -S -O2 -o "$rs/c1.s" "$rs/r.c" || { rm -rf "$rs"; fail "gcc could not emit assembly"; }
env -i PATH="$B2/out/bin:$cpath" "$GXX" -S -O2 -o "$rs/x1.s" "$rs/r.cpp" || { rm -rf "$rs"; fail "g++ could not emit assembly"; }
build_gcc_mesboot1 "$cpath" "$G2" "$B2" "$MM" "$GLD" "$PD" "$GC1" || { rm -rf "$rs"; fail "the second gcc-mesboot1 build did not run"; }
test "$gd1" = "`sha "$GCC1"`" || { rm -rf "$rs"; fail "gcc driver is NOT reproducible"; }
test "$xd1" = "`sha "$GXX"`" || { rm -rf "$rs"; fail "g++ driver is NOT reproducible"; }
env -i PATH="$B2/out/bin:$cpath" "$GCC1" -S -O2 -o "$rs/c2.s" "$rs/r.c" || { rm -rf "$rs"; fail "rebuilt gcc could not emit assembly"; }
env -i PATH="$B2/out/bin:$cpath" "$GXX" -S -O2 -o "$rs/x2.s" "$rs/r.cpp" || { rm -rf "$rs"; fail "rebuilt g++ could not emit assembly"; }
test "`sha "$rs/c1.s"`" = "`sha "$rs/c2.s"`" || { rm -rf "$rs"; fail "the rebuilt gcc emits DIFFERENT assembly — not reproducible"; }
test "`sha "$rs/x1.s"`" = "`sha "$rs/x2.s"`" || { rm -rf "$rs"; fail "the rebuilt g++ emits DIFFERENT assembly — not reproducible"; }
rm -rf "$rs"
echo "   [repro] the gcc + g++ drivers are byte-identical across two builds, and both emit byte-identical assembly (intrinsic output-reproducibility)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, td built GCC 4.6.4 with C AND C++ (guix's"
echo "      gcc-mesboot1): a modern gcc+g++ with static libstdc++, built by gcc-mesboot0 + binutils-mesboot1"
echo "      + make-mesboot against glibc, that compiles+runs C → 42 AND C++ → 42; no gcc/guile/guix, no"
echo "      /gnu/store, reproducible. gcc-mesboot (4.7.4, itself C++) then the final toolchain are next."
