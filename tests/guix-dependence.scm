...
;; i.e. a non-perturbed recipe exists AND recipe-checks proves it builds via
;; `td-builder build-recipe` (behaviorally + reproducibly, no guix/Guile in the
;; build path): corpus package(s), reconstructed toolchain leaves, and library deps.
;; The byte-identity
...
;; recipe-checks covers pkg-config.
;;
...
;; Authored but NOT yet built by td's own builder (not in recipe-checks) — excluded
...
(with-store store
  ...
  (define report
    (string-append
     ...
     "# non-perturbed recipe; proven by recipe-checks). The\n"
     "# byte-identity corpus-* gates were retired with system/td-recipe.scm. pkg-config\n"
     "# is authored but not yet td-built (not in recipe-checks) and is excluded.\n"
