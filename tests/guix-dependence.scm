...
;; i.e. a non-perturbed `tests/ts/recipe-<spec>.ts` exists AND a sibling `td-build-*`
;; own-builder gate proves it builds (behaviorally + reproducibly: td-build,
;; td-build-deps, td-build-resolved, td-build-phases, td-build-corpus,
;; td-build-gettext). The byte-identity `corpus-*` gates that used to ground this
;; were RETIRED when system/td-recipe.scm was dropped (move-off-Guile §5: own, then
;; diverge — the own-builder gates are all-durable, no Guix byte-identity oracle).
;; (The census grounds ownership on the recipe files and asserts each resolves to a
;; real corpus package; it does NOT re-lower the TS recipes here — that proof is the
;; td-build-* gates' job.)
;;
;; EXCLUDED: pkg-config has an authored recipe but no own-builder gate yet — its
;; bundled glib hits a C-standard wall under td's build env — so it is NOT counted
;; as td-reproducible (it would overstate independence). Drop the exclusion once a
;; td-build-pkg-config gate lands.
;;
...
;; Authored but NOT yet built by td's own builder (no td-build-* gate) — excluded
...
(with-store store
  ...
  (define report
    (string-append
     ...
     "# non-perturbed tests/ts/recipe-<spec>.ts; proven by a td-build-* gate). The\n"
