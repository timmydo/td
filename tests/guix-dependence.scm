...
;;
;; A derivation is "td-reproducible" iff td reconstructs it store-path-equal to
;; the corpus oracle — i.e. a non-perturbed `tests/ts/recipe-<spec>.ts` exists.
;; Each such recipe is proven store-path-equal by a sibling `corpus-*` gate in
;; the same ladder, so in a GREEN loop "recipe exists" ⇒ "td reproduces it".
;; (v1 grounds ownership on the recipe files and asserts each resolves to a real
;; corpus package; it does NOT re-lower the TS recipes here — that proof is the
;; corpus-* gates' job. Stronger in-census binding is a documented follow-up.)
;;
...
(with-store store
  ...
  (define report
    (string-append
     ...
     "# td-reproducible = td reconstructs the derivation store-path-equal to the\n"
     "# corpus oracle (a non-perturbed tests/ts/recipe-<spec>.ts; proven by corpus-*).\n"
