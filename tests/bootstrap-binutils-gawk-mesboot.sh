#!/bin/sh
# tests/bootstrap-binutils-gawk-mesboot.sh — source-bootstrap BRICK 5 (gcc toolchain): the gcc-mesboot1
# (GCC 4.6.4, C++) toolchain rebuilds GNU Binutils 2.20.1a AND builds GNU awk 3.1.8 — guix's
# binutils-mesboot + gawk-mesboot. These are the two tools the next C library (glibc-mesboot 2.16.0)
# needs: binutils-mesboot is binutils rebuilt by the c++-capable gcc; gawk-mesboot is a real GNU awk
# (glibc 2.16.0's versions.awk is too complex for the seed's gash-utils awk).
#
# From the 229-byte seed, td builds the whole chain (bricks 0-4 + make/patch/binutils/gcc/glibc rungs +
# gcc-mesboot0 + binutils-mesboot1 + make-mesboot + gcc-mesboot1), then with gcc-mesboot1 as CC builds
# binutils-mesboot (plain configure, like binutils-mesboot1) and gawk-mesboot (ac_cv_func_connect=no,
# `make gawk`). Static throughout (td's glibc is static-only); kernel UAPI headers host-produced.
#
# i686, static, serial. mes/…/gcc-4.6.4/gcc-g++/gmp/mpfr/mpc/gawk-3.1.8 are td-fetched; the 5 guix boot
# patches are vendored source data — as/ld/gawk are compiled from source (no guix bytes; [no-guix] verifies).
#
# Legs (DURABLE):
#   [pinned-input] the chain's tarballs + 5 boot patches + gcc-4.6.4/gcc-g++/gmp/mpfr/mpc + gawk-3.1.8 match sha256.
#   [no-guix]      built on a curated PATH with gcc/g++/cc/guile/guix DENIED; no /gnu/store in as/ld/gawk.
#   [behavioral]   binutils-mesboot as/ld report 2.20 and (with gcc-mesboot1) assemble+link+RUN C → 42;
#                  gawk-mesboot reports 3.1.8 and processes text ('{print $2}' → beta) + sums 10+20+12 → 42.
#   [repro]        two independent builds (same dir) yield byte-identical as + ld + gawk.
set -eu
...
# --- [pinned-input] extras: make-3.82 + gcc-4.6.4 + gcc-g++ + gmp/mpfr/mpc + gawk-3.1.8 + the patch ---
...
for pair in "$MAKE382_TB:`lf "$MAKE382_LOCK" sha256`" "$GCC464_TB:`lf "$GCC464_LOCK" sha256`" "$GPP464_TB:`lf "$GPP464_LOCK" sha256`" \
            "$GMP_TB:`lf "$GMP_LOCK" sha256`" "$MPFR_TB:`lf "$MPFR_LOCK" sha256`" "$MPC_TB:`lf "$MPC_LOCK" sha256`" "$GAWK_TB:`lf "$GAWK_LOCK" sha256`"; do
  f=${pair%:*}; want=${pair##*:}
  test -f "$f" || fail "pinned tarball not warm ($f) — run 'td-feed warm sources'"
  test "`sha "$f"`" = "$want" || fail "warmed $f sha256 != lock pin ($want)"
done
...
echo "   [pinned-input] + make-3.82, gcc-4.6.4, gcc-g++, gmp/mpfr/mpc, gawk-3.1.8 + the gcc-4.6.4 boot patch match their pins"
...
BO=`mktemp -d`/binutilsmesbootbuild; build_binutils_mesboot "$cpath" "$GM1" "$B2" "$GLD" "$MM" "$PD" "$BO" || fail "gcc-mesboot1 did not rebuild binutils"
GO=`mktemp -d`/gawkmesbootbuild; build_gawk_mesboot "$cpath" "$GM1" "$B2" "$GLD" "$MM" "$GO" || fail "gcc-mesboot1 did not build GNU awk"
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BO"`" "`dirname "$GO"`" "`dirname "$cpath"`"' EXIT INT TERM

AS="$BO/out/bin/as"; LD="$BO/out/bin/ld"; GAWK="$GO/out/bin/gawk"; GM1DIR="$GM1/out/lib/gcc/i686-unknown-linux-gnu/4.6.4"

# --- [no-guix] -------------------------------------------------------------------------------
for art in "$AS" "$LD" "$GAWK"; do test -x "$art" || fail "missing $art"; if grep -q -a '/gnu/store' "$art"; then fail "$art contains /gnu/store bytes"; fi; done
echo "   [no-guix] seed → … → gcc-mesboot1 → {binutils-mesboot, gawk-mesboot} built with no gcc/guile/guix on PATH; no /gnu/store in as/ld/gawk"

# --- [behavioral] binutils-mesboot as/ld → 2.20 + round-trip C → 42; gawk-mesboot → 3.1.8 + text ------
av=`env -i "$AS" --version 2>"$BO/as.err" | head -1` || { tail -3 "$BO/as.err" >&2; fail "binutils-mesboot as did not run"; }
echo "$av" | grep -q '2\.20' || fail "binutils-mesboot as --version gave [$av], want 2.20.x"
lv=`env -i "$LD" --version 2>"$BO/ld.err" | head -1` || { tail -3 "$BO/ld.err" >&2; fail "binutils-mesboot ld did not run"; }
echo "$lv" | grep -q '2\.20' || fail "binutils-mesboot ld --version gave [$lv], want 2.20.x"
wd=`mktemp -d`; printf '#include <stdio.h>\nint main(){printf("hi");return 42;}\n' > "$wd/t.c"
( cd "$wd" && env -i PATH="$BO/out/bin:$cpath" C_INCLUDE_PATH="$GLD/out/include" LIBRARY_PATH="$GLD/out/lib:$GM1DIR" \
    "$GM1/out/bin/gcc" -static -B"$GLD/out/lib" -o t t.c ) >"$wd/cc.log" 2>&1 \
  || { tail -10 "$wd/cc.log" >&2; rm -rf "$wd"; fail "binutils-mesboot as+ld could not assemble+link a C program"; }
set +e; ( cd "$wd" && env -i ./t ); rc=$?; set -e; rm -rf "$wd"
test "$rc" = 42 || fail "the program assembled+linked by binutils-mesboot returned $rc, want 42"
gv=`env -i "$GAWK" --version 2>"$GO/gawk.err" | head -1` || { tail -3 "$GO/gawk.err" >&2; fail "gawk-mesboot did not run"; }
echo "$gv" | grep -q '3\.1\.8' || fail "gawk-mesboot --version gave [$gv], want 3.1.8"
f2=`printf 'alpha beta gamma\n' | env -i "$GAWK" '{print $2}'`; test "$f2" = beta || fail "gawk '{print \$2}' gave [$f2], want beta"
sm=`printf '10\n20\n12\n' | env -i "$GAWK" '{s+=$1} END{print s}'`; test "$sm" = 42 || fail "gawk sum gave [$sm], want 42"
echo "   [behavioral] as→'$av', ld→'$lv' assembled+linked+ran C → 42; gawk→'$gv' printed field 2 (beta) + summed 10+20+12 → 42"

# --- [repro] a second independent build (same dir) yields byte-identical as + ld + gawk --------------
a1=`sha "$AS"`; l1=`sha "$LD"`; w1=`sha "$GAWK"`
build_binutils_mesboot "$cpath" "$GM1" "$B2" "$GLD" "$MM" "$PD" "$BO" || fail "the second binutils-mesboot build did not run"
build_gawk_mesboot "$cpath" "$GM1" "$B2" "$GLD" "$MM" "$GO" || fail "the second gawk-mesboot build did not run"
test "$a1" = "`sha "$AS"`" || fail "binutils-mesboot as is NOT reproducible"
test "$l1" = "`sha "$LD"`" || fail "binutils-mesboot ld is NOT reproducible"
test "$w1" = "`sha "$GAWK"`" || fail "gawk-mesboot gawk is NOT reproducible"
echo "   [repro] two independent builds produce byte-identical as + ld + gawk (reproducible)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, the gcc-mesboot1 (c++) toolchain rebuilt"
echo "      GNU Binutils 2.20.1a AND built GNU awk 3.1.8 (guix's binutils-mesboot + gawk-mesboot): as+ld"
echo "      assemble+link+run C → 42 and gawk processes text + sums → 42; no gcc/guile/guix, no /gnu/store,"
echo "      reproducible. These two feed glibc-mesboot 2.16.0, then gcc-mesboot (GCC 4.9), the final mesboot gcc."
