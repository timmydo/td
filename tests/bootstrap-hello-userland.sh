#!/bin/sh
# tests/bootstrap-hello-userland.sh — source-bootstrap BRICK 6 (rung 3): the FIRST real userland package
# built from source by the dynamic /td/store toolchain. From the 229-byte seed, td builds the chain →
# gcc-mesboot1 + binutils-mesboot → a SHARED glibc 2.16.0, interns them content-addressed into /td/store,
# then compiles a REAL autotools package — GNU hello 2.10 — from source (a full ./configure && make, the
# unmodified upstream build system) with that /td/store toolchain. The resulting `hello` is a DYNAMIC ELF
# whose interpreter + RUNPATH point at the /td/store glibc; it is interned at /td/store and RUN in the
# store-ns own-root (/gnu/store ABSENT) → prints "Hello, world!". First time a from-source GNU program runs
# from /td/store, unmixed from guix — proof the toolchain isn't just a self-test but builds real software.
#
# The build runs in the loop sandbox (where configure/make/sed/awk live) via a build-wrapper gcc: it points
# -isystem/-B/-L at the live build-dir toolchain (must exist at link) but bakes the /td/store interpreter +
# RUNPATH (header strings — needn't exist at link). hello is built CROSS-style (--build != --host) so
# configure never runs a target binary (which would need /td/store). This is the same composition as the
# rung-2 /td/store wrapper, applied to a real package. all sources td-fetched.
#
# i686, serial.
#
# Legs (DURABLE — no guix oracle in any):
#   [pinned-input] chain tarballs + boot patches + glibc-2.16.0 + hello-2.10 match sha256.
#   [no-guix]      no /gnu/store bytes in the shared libc.so.6 NOR in the built `hello` binary.
#   [content-addr] the interned paths are /td/store/<nar-hash>-<name>.
#   [behavioral]   the /td/store toolchain compiles GNU hello 2.10 from source (configure + make); the
#                  binary is DYNAMIC with interp = the /td/store ld-linux.so.2, and RUNS in the own-root
#                  → "Hello, world!". (A real GNU program, built + run from /td/store.)
...
# --- [pinned-input] extras: chain sources + gawk + glibc-2.16.0 + 2 patches ------------------------
...
HELLO_LOCK=`ls seed/sources/hello-2.10.lock`; HELLO_TB=".td-build-cache/sources/`lf "$HELLO_LOCK" file`"
...
for pair in "$MAKE382_TB:`lf "$MAKE382_LOCK" sha256`" "$GCC464_TB:`lf "$GCC464_LOCK" sha256`" "$GPP464_TB:`lf "$GPP464_LOCK" sha256`" \
            ...
            "$GAWK_TB:`lf "$GAWK_LOCK" sha256`" "$GLIBC216_TB:`lf "$GLIBC216_LOCK" sha256`" \
            "$HELLO_TB:`lf "$HELLO_LOCK" sha256`"; do
  f=${pair%:*}; want=${pair##*:}
  test -f "$f" || fail "pinned tarball not warm ($f) — run 'td-feed warm sources'"
  test "`sha "$f"`" = "$want" || fail "warmed $f sha256 != lock pin ($want)"
done
...
echo "   [pinned-input] chain + gawk-3.1.8/glibc-2.16.0/hello-2.10 + the boot patches match their pins"
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BMB"`" "`dirname "$GAWKMB"`" "`dirname "$GSH"`" "${snwork:-/nonexistent}" "`dirname "$cpath"`"' EXIT INT TERM

GLS="$GSH/out"
test -e "$GLS/lib/libc.so.6" -a -e "$GLS/lib/ld-linux.so.2" || fail "shared glibc missing libc.so.6/ld-linux.so.2"
if grep -q -a '/gnu/store' "$GLS/lib/libc.so.6"; then fail "$GLS/lib/libc.so.6 contains /gnu/store bytes"; fi
echo "   [no-guix] seed → … → gcc-mesboot1 → SHARED glibc 2.16.0; no /gnu/store in libc.so.6"
...
GLP=`"$TB" store-add-recursive glibc-shared "$GLS" "$store" "$sndb"` || fail "store-add-recursive glibc-shared failed"
GCP=`"$TB" store-add-recursive gcc-mesboot1 "$GM1/out" "$store" "$sndb"` || fail "store-add-recursive gcc-mesboot1 failed"
BUP=`"$TB" store-add-recursive binutils-mesboot "$BMB/out" "$store" "$sndb"` || fail "store-add-recursive binutils-mesboot failed"
case "$GLP" in /td/store/*-glibc-shared) ;; *) fail "glibc-shared not content-addressed at /td/store: $GLP" ;; esac
echo "   [content-addr] interned $GLP + gcc-mesboot1 + binutils-mesboot, content-addressed in /td/store"

glrel=${GLP#/td/store/}; gcrel=${GCP#/td/store/}; burel=${BUP#/td/store/}

# --- BUILD GNU hello 2.10 from source with the /td/store toolchain ---------------------------------
# The build runs here in the loop sandbox (configure/make/sed live here); the build-wrapper gcc points
# -isystem/-B/-L at the LIVE build-dir toolchain (must exist at link) but bakes the /td/store interp +
# RUNPATH (header strings — needn't exist at link). awk (config.status generates the Makefile with it) is
# globbed from the exposed /gnu/store, like the chain's build tools; the OUTPUT hello is what [no-guix]
# guards. CROSS-style (--build != --host) so configure never runs a target binary (which needs /td/store).
GM1DIR="$GM1/out/lib/gcc/i686-unknown-linux-gnu/4.6.4"
SH=`command -v bash 2>/dev/null || command -v sh`; test -n "$SH" || fail "no shell in the sandbox"
awkb=`command -v awk 2>/dev/null || ls /gnu/store/*gawk*/bin/awk 2>/dev/null | sort | head -1`
test -n "$awkb" || fail "no awk for config.status"; awkdir=`dirname "$awkb"`

wrap=`mktemp -d`/wrap; mkdir -p "$wrap"
cat > "$wrap/gcc" <<WRAP
#!$SH
exec "$GM1/out/bin/gcc" -isystem "$GLS/include" -B"$GLS/lib" -L"$GLS/lib" -L"$GM1DIR" \\
  -Wl,--dynamic-linker -Wl,/td/store/$glrel/lib/ld-linux.so.2 -Wl,-rpath -Wl,/td/store/$glrel/lib "\$@"
WRAP
chmod 0555 "$wrap/gcc"

hsrc=`mktemp -d`/hello; mkdir -p "$hsrc"
tar -xzf "$HELLO_TB" -C "$hsrc" --strip-components=1 || fail "hello-2.10 unpack failed"
( cd "$hsrc"
  env PATH="$BMB/out/bin:$awkdir:$PATH" CONFIG_SHELL="$SH" SHELL="$SH" CC="$wrap/gcc" \
    "$SH" ./configure --build=i686-pc-linux-gnu --host=i686-unknown-linux-gnu --disable-nls >cfg.log 2>&1 \
    || { echo "hello configure failed:" >&2; cp cfg.log "$ROOT/.td-build-cache/_hello-cfg.log" 2>/dev/null||true; tail -20 cfg.log >&2; exit 1; }
  env PATH="$BMB/out/bin:$awkdir:$PATH" CONFIG_SHELL="$SH" SHELL="$SH" make >mk.log 2>&1 \
    || { echo "hello make failed:" >&2; cp mk.log "$ROOT/.td-build-cache/_hello-mk.log" 2>/dev/null||true; tail -25 mk.log >&2; exit 1; } ) \
  || fail "GNU hello did not build with the /td/store toolchain"
# GNU hello 2.10 links its binary as ./hello in the package root (Makefile.am: bin_PROGRAMS = hello; -o hello).
test -x "$hsrc/hello" || { tail -20 "$hsrc/mk.log" >&2; fail "no hello binary produced"; }
if grep -q -a '/gnu/store' "$hsrc/hello"; then fail "the built hello binary contains /gnu/store bytes"; fi
hi=`"$BMB/out/bin/readelf" -l "$hsrc/hello" 2>/dev/null | grep -o "/td/store/$glrel/lib/ld-linux.so.2" | head -1`
test -n "$hi" || fail "the built hello is not dynamic against the /td/store ld-linux.so.2"
echo "   [no-guix] the /td/store toolchain compiled GNU hello 2.10 from source → dynamic ELF, interp=$hi, no /gnu/store bytes"

# Intern hello at /td/store (its own dir).
mkdir -p "$store/hello/bin"; cp "$hsrc/hello" "$store/hello/bin/hello"; chmod -R u+w "$store"
HLP=`"$TB" store-add-recursive hello "$store/hello" "$store" "$sndb"` || fail "store-add-recursive hello failed"
case "$HLP" in /td/store/*-hello) ;; *) fail "hello not content-addressed at /td/store: $HLP" ;; esac
hlrel=${HLP#/td/store/}
echo "   [content-addr] interned $HLP, content-addressed in /td/store"
...
/td/store/'"$hlrel"'/bin/hello
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns own-root probe exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2
case "$snout" in *"Hello, world!"*) ;; *) fail "hello output unexpected: $snout" ;; esac
echo "   [behavioral] GNU hello, built from source by the /td/store toolchain, ran in the own-root → \"Hello, world!\""
...
echo "PASS: source-bootstrap brick 6 (rung 3) — from the 229-byte seed, td built a SHARED glibc 2.16.0 +"
echo "      gcc-mesboot1 + binutils-mesboot at /td/store, then compiled a REAL autotools package — GNU"
echo "      hello 2.10 — from source (./configure && make) with that toolchain. The binary is DYNAMIC"
echo "      against the /td/store glibc and RUNS in the own-root → \"Hello, world!\", /gnu/store ABSENT."
echo "      First from-source GNU userland program built + run from /td/store, unmixed from guix."
