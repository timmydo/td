#!/bin/sh
# tests/bootstrap-gcc-mesboot-494-store-native.sh — source-bootstrap BRICK 6/7 (the FINAL modern toolchain,
# rung B0 — the bridge): gcc-mesboot GCC 4.9.4 (the FINAL mesboot gcc, WITH C++) + a C++-capable wrapper at
# the dynamic /td/store. The 4.6.4 /td/store wrapper builds C (binutils 2.44, rung A), but modern gcc-boot0
# = gcc 14.3.0 needs C++14 — so this rung bridges /td/store to 4.9.4 (full C++11), the compiler gcc-boot0
# will use. NEW capability: C++ at /td/store.
#
...
# glibc-mesboot 2.16.0 STATIC), then builds GCC 4.9.4 from one tarball (gmp/mpfr/mpc in-tree), AND a SHARED
# glibc 2.16.0 (the runtime glibc the dynamic wrapper links against). It interns 4.9.4 + the shared glibc +
# binutils-mesboot content-addressed into /td/store, and generates a gcc/g++ WRAPPER there: a PLAIN
# invocation produces a DYNAMIC /td/store binary (interp + RUNPATH = /td/store; headers/crt/libc baked;
# libstdc++/libgcc linked static since 4.9.4 is static). Proven in the store-ns own-root (/gnu/store ABSENT):
# the wrapped gcc AND g++ compile a C and a C++ program → both dynamic, interp=/td/store, run → 42.
#
# 4.9.4 itself is built STATIC against the static glibc 2.16.0 (the proven #185 recipe); the wrapper then
# links DYNAMIC against the SHARED glibc 2.16.0 (both glibc 2.16.0, ABI-compatible). i686, serial. all
# sources td-fetched. (4.9.4's own reproducibility is guarded by the bootstrap-gcc-mesboot gate, #185.)
#
# Legs (DURABLE — no guix oracle in any):
#   [pinned-input] chain tarballs + vendored boot patches + gcc-4.9.4 match sha256.
#   [no-guix]      built with gcc/g++/cc/guile/guix DENIED; no /gnu/store in gcc/g++/cpp/cc1 NOR libc.so.6.
#   [content-addr] the interned paths are /td/store/<nar-hash>-<name>.
#   [behavioral]   the /td/store gcc-mesboot-4.9.4 wrapper, called PLAINLY, compiles a DYNAMIC C AND C++
#                  program (interp = /td/store ld-linux.so.2); both RUN in the own-root → 42 — C++ at /td/store.
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BMB"`" "`dirname "$GAWKMB"`" "`dirname "$GOUT"`" "`dirname "$GMB"`" "`dirname "$GSH"`" "`dirname "$cpath"`"' EXIT INT TERM
...
# --- store-native: build the SHARED glibc 2.16.0, intern 4.9.4 + shared glibc at /td/store, wrap it ---
GLS="$GSH/out"
...
GCP=`"$TB" store-add-recursive gcc-mesboot-4.9.4 "$GCCM" "$store" "$sndb"` || fail "store-add gcc-mesboot-4.9.4 failed"
BUP=`"$TB" store-add-recursive binutils-mesboot "$BMB/out" "$store" "$sndb"` || fail "store-add binutils-mesboot failed"
case "$GCP" in /td/store/*-gcc-mesboot-4.9.4) ;; *) fail "gcc-mesboot-4.9.4 not content-addressed at /td/store: $GCP" ;; esac
...
printf 'struct A{int v; A(int x):v(x){} int f(){return v;}};\nint main(){A a(42); return a.f();}\n' > "$store/work/cpp.cc"

# The /td/store gcc-mesboot-4.9.4 wrapper: PLAIN gcc/g++ baking the /td/store dynamic build (headers, crt
# -B, libc/libgcc -L, dynamic-linker + rpath). C++ links libstdc++/libgcc STATIC (4.9.4 is static) so the
# program needs only the /td/store glibc at runtime. The C++-capable wrapper modern gcc-boot0 will use.
...
exec /td/store/$gcrel/bin/$cc \\
  ...
  -B/td/store/$glrel/lib -L/td/store/$glrel/lib -L/td/store/$gcrel/lib/gcc/i686-unknown-linux-gnu/4.9.4 \\
...
echo "   generated the gcc-mesboot-4.9.4 wrapper (gcc + g++) at /td/store/wrapper"

# own-root: the wrapper, called PLAINLY, builds a DYNAMIC C AND C++ program (interp=/td/store) that runs.
snscript='
set -e
...
/td/store/wrapper/g++ -o /tmp/cpp /td/store/work/cpp.cc && echo CPP-COMPILED
...
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns 4.9.4 wrapper probe exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2
echo "$snout" | grep -q '^C-COMPILED$'         || fail "the /td/store 4.9.4 wrapper did not compile a C program"
...
echo "$snout" | grep -q '^CPP-COMPILED$'       || fail "the /td/store 4.9.4 wrapper did not compile a C++ program"
echo "$snout" | grep -q '^CPP-INTERP-TDSTORE$' || fail "the C++ program interpreter is not /td/store"
echo "$snout" | grep -q '^CPPRC=42$'           || fail "the C++ program did not return 42"
echo "   [behavioral] the /td/store gcc-mesboot-4.9.4 wrapper, called PLAINLY, built a DYNAMIC C AND C++ program (interp=/td/store ld-linux.so.2), both ran → 42 — C++ at /td/store"
...
echo "PASS: source-bootstrap brick 6/7 (final toolchain, rung B0 — the bridge) — from the 229-byte seed, td"
echo "      built the chain → gcc-mesboot1 + binutils-mesboot + glibc 2.16.0 (static AND shared) → GCC 4.9.4"
echo "      (the FINAL mesboot gcc, with C++), interned 4.9.4 + the shared glibc at /td/store, and a gcc/g++"
echo "      WRAPPER there compiles PLAINLY a DYNAMIC C AND C++ program that runs in the own-root → 42,"
echo "      /gnu/store ABSENT. C++ at /td/store — the compiler that builds modern gcc-boot0 (gcc 14.3.0)."
