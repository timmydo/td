#!/bin/sh
# tests/bootstrap-gcc-mesboot0.sh — source-bootstrap BRICK 5 (gcc toolchain): GCC 2.95.3 REBUILT by the
# first gcc against glibc — guix's gcc-mesboot0. After glibc-mesboot0 (the C library), td rebuilds gcc
# so the compiler is self-hosted on glibc (not the mes libc the tcc-built gcc-core-mesboot0 used). This
# is the toolchain re-baseline that gcc-mesboot1 (4.6.4) is then built on.
#
# From the 229-byte seed, td builds the whole chain through gcc-core-mesboot0 + glibc-mesboot0 (bricks
# 0-4 + make/patch/binutils/gcc/glibc rungs), then the td-built `patch` re-applies guix's gcc boot patch
# and the FIRST gcc (gcc-core-mesboot0, CC=<gcc> not tcc) rebuilds GCC 2.95.3, now resolving headers/libs
# to glibc + the first gcc's gcc-lib. Setup matching guix gcc-mesboot0: config.cache float hint,
# RANLIB=true, LANGUAGES=c, a simpler install2 (no libtcc1). Plus the make-in-sandbox fixes (SHELL var,
# cleared MAKEFLAGS), the /bin/sh-shebang rewrite, and the seed gcc's cpp on PATH.
#
...
# linux-4.14.67 source on the HOST (td-feed warm kernel-headers i386, run in check.sh's prelude — like
# warm-tsgo); this gate CONSUMES the produced tarball. North-star-clean: standard UAPI text from source.
#
# i686, static, serial. mes/nyacc/tcc/make/patch/binutils/gcc/glibc/linux are td-fetched; the 4 guix
# boot patches are vendored source data — gcc/cc1 are compiled from gcc source + the patch (no
...
#   [behavioral]   the glibc-based gcc reports 2.95.3 and COMPILES + LINKS + RUNS a C program → 42 — a
#                  gcc self-hosted on glibc, not mes libc.
#   [repro]        two independent gcc-mesboot0 builds (same dir) yield a byte-identical gcc + cc1.
set -eu
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$cpath"`"' EXIT INT TERM

GCC2="$G2/out/bin/gcc"; G2DIR="$G2/out/lib/gcc-lib/i686-unknown-linux-gnu/2.95.3"

# --- [no-guix] -------------------------------------------------------------------------------
test -f "$G2DIR/cc1" || fail "gcc-mesboot0 built but cc1 missing at $G2DIR/cc1"
for art in "$GCC2" "$G2DIR/cc1"; do if grep -q -a '/gnu/store' "$art"; then fail "$art contains /gnu/store bytes"; fi; done
echo "   [no-guix] seed → … → glibc → gcc-mesboot0 (gcc 2.95.3 on glibc) built with no gcc/guile/guix on PATH; no /gnu/store in gcc/cc1"

# --- [behavioral] the glibc-based gcc reports 2.95.3 and compiles+links+runs a C program → 42 -------
gv=`env -i "$GCC2" --version 2>"$G2/gcc.err" | head -1` || { tail -3 "$G2/gcc.err" >&2; fail "gcc-mesboot0 did not run"; }
echo "$gv" | grep -q '2.95.3' || fail "gcc-mesboot0 --version gave [$gv], want 2.95.3"
wd=`mktemp -d`; printf '#include <stdio.h>\nint main(){printf("hi");return 42;}\n' > "$wd/t.c"
( cd "$wd" && env PATH="$G2/bin:$cpath" C_INCLUDE_PATH="$GLD/out/include:$G2DIR/include" LIBRARY_PATH="$GLD/out/lib:$G2DIR" \
    "$GCC2" -static -B"$GLD/out/lib" -o t t.c ) >"$wd/cc.log" 2>&1 \
  || { tail -10 "$wd/cc.log" >&2; rm -rf "$wd"; fail "gcc-mesboot0 could not compile+link a C program"; }
head -c20 "$wd/t" | od -An -tx1 | grep -q '7f 45 4c 46 01' || { rm -rf "$wd"; fail "gcc-mesboot0 did not emit a 32-bit ELF"; }
set +e; ( cd "$wd" && env -i ./t ); rc=$?; set -e
rm -rf "$wd"
test "$rc" = 42 || fail "the gcc-mesboot0-built program returned $rc, want 42"
echo "   [behavioral] gcc-mesboot0→'$gv'; it compiled+linked+ran a C program → 42 — a gcc self-hosted on glibc, not mes libc"

# --- [repro] a second independent gcc-mesboot0 build (same dir) is byte-identical -------------------
g1=`sha "$GCC2"`; c1=`sha "$G2DIR/cc1"`
build_gcc_mesboot0 "$cpath" "$GD" "$BD" "$GLD" "$HD" "$MK" "$PD" "$G2" || fail "the second gcc-mesboot0 build did not run"
test "$g1" = "`sha "$GCC2"`" || fail "gcc-mesboot0 gcc is NOT reproducible — r1=$g1 r2=`sha "$GCC2"`"
test "$c1" = "`sha "$G2DIR/cc1"`" || fail "gcc-mesboot0 cc1 is NOT reproducible — r1=$c1 r2=`sha "$G2DIR/cc1"`"
echo "   [repro] two independent gcc-mesboot0 builds produce a byte-identical gcc + cc1 (reproducible)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, td rebuilt GCC 2.95.3 against glibc"
echo "      (guix's gcc-mesboot0): a gcc self-hosted on glibc that compiles+links+runs C → 42; no"
