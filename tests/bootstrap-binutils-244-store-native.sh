#!/bin/sh
# tests/bootstrap-binutils-244-store-native.sh — source-bootstrap BRICK 6/7 (the FINAL modern toolchain,
# rung A): the FIRST modern toolchain component built from source by the dynamic /td/store toolchain. From
# the 229-byte seed, td builds the chain → gcc-mesboot1 (GCC 4.6.4) + binutils-mesboot → a SHARED glibc
# 2.16.0, interns them content-addressed into /td/store, then builds MODERN GNU Binutils 2.44 from source (a
# full ./configure && make, the unmodified upstream build system) with that /td/store toolchain. The built
# as/ld/ar are DYNAMIC ELFs whose interp + RUNPATH point at the /td/store glibc; they are interned at
# /td/store and RUN in the store-ns own-root (/gnu/store ABSENT): they report version 2.44 AND actually
# assemble+link a C program (driven via gcc-mesboot1 -B at the new binutils) that returns 42. This is the
# binutils-boot0 of guix's final-toolchain ladder, made td-native — the first MODERN component at /td/store.
#
# The build runs in the loop sandbox via TWO build-wrappers (the userland build-wrapper trick scaled to a
# modern recursive toolchain): CC (wrap) compiles the TARGET as/ld with /td/store interp+RUNPATH baked;
# CC_FOR_BUILD (wrapb) compiles the in-tree BUILD tools (chew &c.) that must RUN in the sandbox now, so it
# bakes the LIVE build-dir interp instead. gcc-mesboot1 4.6.4 needs -std=gnu99 (default gnu89; binutils 2.44
# is C99+). CROSS-style (--build != --host) so configure never runs a target binary. --disable-gold (gold is
# C++; ld.bfd is td's linker — own-then-diverge from guix's --enable-gold). all sources td-fetched.
#
# i686, serial.
#
...
for pair in "$MAKE382_TB:`lf "$MAKE382_LOCK" sha256`" "$GCC464_TB:`lf "$GCC464_LOCK" sha256`" "$GPP464_TB:`lf "$GPP464_LOCK" sha256`" \
            ...
            "$BU244_TB:`lf "$BU244_LOCK" sha256`"; do
  f=${pair%:*}; want=${pair##*:}
  test -f "$f" || fail "pinned tarball not warm ($f) — run 'td-feed warm sources'"
  test "`sha "$f"`" = "$want" || fail "warmed $f sha256 != lock pin ($want)"
done
...
echo "   [pinned-input] chain + gawk-3.1.8/glibc-2.16.0/binutils-2.44 + the boot patches match their pins"
...
# --- BUILD modern GNU Binutils 2.44 from source with the /td/store toolchain -----------------------
# The build runs here in the loop sandbox via TWO build-wrappers (the userland trick scaled to a modern
# recursive autotools toolchain). CC (wrap) compiles the TARGET tools (as/ld/…): real -isystem/-B/-L at the
# live build-dir toolchain (must exist at link) + /td/store interp+RUNPATH baked (header strings). CC_FOR_BUILD
# (wrapb) compiles the in-tree BUILD tools (chew &c.) that must RUN in the sandbox NOW, so it bakes the LIVE
# build-dir interp instead. gcc-mesboot1 4.6.4 needs -std=gnu99 (its default is gnu89; binutils 2.44 is C99+).
# CROSS-style (--build != --host) so configure never runs a target binary (which needs /td/store). --disable-gold
# (gold is C++; ld.bfd is td's linker — own-then-diverge from guix's --enable-gold). MAKEINFO=true skips the
# texinfo docs (need makeinfo + /bin/sh-shebang scripts absent in the sandbox). xz/awk/flex/bison/cmp are build
# scaffolding globbed from /gnu/store (like the chain); the [no-guix] leg guards the OUTPUT as/ld.
...
test -n "$xzb" || fail "no xz to unpack binutils-2.44.tar.xz"
toolbin=`mktemp -d`/tools; mkdir -p "$toolbin"
for pair in awk:gawk flex:flex bison:bison cmp:diffutils diff:diffutils; do
  n=${pair%%:*}; pk=${pair##*:}; b=`command -v "$n" 2>/dev/null || ls /gnu/store/*$pk*/bin/$n 2>/dev/null | sort | head -1`
  test -n "$b" && ln -sf "$b" "$toolbin/$n" || true
done
ln -sf "$toolbin/flex" "$toolbin/lex" 2>/dev/null || true; ln -sf "$toolbin/bison" "$toolbin/yacc" 2>/dev/null || true

wrap=`mktemp -d`/wrap; mkdir -p "$wrap"
cat > "$wrap/gcc" <<WRAP
#!$SH
exec "$GM1/out/bin/gcc" -std=gnu99 -isystem "$GLS/include" -B"$GLS/lib" -L"$GLS/lib" -L"$GM1DIR" \\
  -Wl,--dynamic-linker -Wl,/td/store/$glrel/lib/ld-linux.so.2 -Wl,-rpath -Wl,/td/store/$glrel/lib "\$@"
WRAP
wrapb=`mktemp -d`/wrapb; mkdir -p "$wrapb"
cat > "$wrapb/gcc" <<WRAPB
#!$SH
exec "$GM1/out/bin/gcc" -std=gnu99 -isystem "$GLS/include" -B"$GLS/lib" -L"$GLS/lib" -L"$GM1DIR" \\
  -Wl,--dynamic-linker -Wl,$GLS/lib/ld-linux.so.2 -Wl,-rpath -Wl,$GLS/lib "\$@"
WRAPB
...
case "$BUP244" in /td/store/*-binutils-2.44) ;; *) fail "binutils-2.44 not content-addressed at /td/store: $BUP244" ;; esac
b244rel=${BUP244#/td/store/}
echo "   [content-addr] interned $BUP244, content-addressed in /td/store"
...
/td/store/'"$b244rel"'/bin/ld --version
/td/store/'"$b244rel"'/bin/as --version
export PATH=/td/store/'"$b244rel"'/bin
/td/store/'"$gcrel"'/bin/gcc -B/td/store/'"$b244rel"'/bin -B/td/store/'"$glrel"'/lib \
  -Wl,--dynamic-linker -Wl,/td/store/'"$glrel"'/lib/ld-linux.so.2 -Wl,-rpath -Wl,/td/store/'"$glrel"'/lib \
  /td/store/work/t.c -o /tmp/t && echo LINKED-BY-244
set +e; /tmp/t; echo "RC=$?"; set -e
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns own-root binutils-2.44 probe exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2
echo "$snout" | grep -q 'GNU ld.*2\.44'   || fail "ld --version did not report 2.44 in the own-root"
echo "$snout" | grep -q 'assembler.*2\.44' || fail "as --version did not report 2.44 in the own-root"
echo "$snout" | grep -q '^LINKED-BY-244$'  || fail "the modern /td/store as+ld did not assemble+link a program"
echo "$snout" | grep -q '^RC=42$'          || fail "the program linked by binutils 2.44 did not return 42"
echo "   [behavioral] modern binutils 2.44, built from source by the /td/store toolchain, runs in the own-root — as/ld report 2.44 AND link a program that returns 42"
...
echo "PASS: source-bootstrap brick 6/7 (final toolchain, rung A) — from the 229-byte seed, td built the chain"
echo "      → gcc-mesboot1 + binutils-mesboot + SHARED glibc 2.16.0 at /td/store, then built a MODERN GNU"
echo "      Binutils 2.44 from source (./configure && make) with that toolchain. The as/ld are DYNAMIC against"
echo "      the /td/store glibc, interned at /td/store, and RUN in the own-root: they report 2.44 AND assemble+"
echo "      link a program that returns 42, /gnu/store ABSENT. First MODERN toolchain component at /td/store —"
echo "      the binutils-boot0 of guix's final-toolchain ladder, td-native (gcc-boot0 next)."
