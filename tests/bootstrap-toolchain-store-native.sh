#!/bin/sh
# tests/bootstrap-toolchain-store-native.sh — source-bootstrap: place the seed-built STATIC mesboot
# toolchain at td's OWN store `/td/store` and prove it COMPILES + RUNS from there in td's own root,
# with `/gnu/store` ABSENT. The first `/td/store`-native step (after the chain reaches GCC 4.9.4):
# the toolchain's bytes are already guix-free (the bootstrap gates' [no-guix] legs prove no /gnu/store
# in the binaries) and STATIC (no RUNPATH), so they need NO relocation — `store-add-recursive` interns
# them content-addressed into `/td/store`, and `store-ns` runs them in a rootless userns where
# `/td/store` IS the store and `/gnu/store`/guix are absent. This is the registered `/td/store` path
# td-subst can serve (chain-caching), and the unmixed base the userland is built on next.
#
# From the 229-byte seed, td builds the whole chain → gcc-mesboot (4.9.4) + binutils-mesboot + glibc-
# mesboot (2.16.0); then the guix-free stage0 td-builder interns those three trees into `/td/store`
# and runs gcc-mesboot THERE to compile+link a static C program that returns 42.
#
# Legs (DURABLE — no guix oracle in any of them):
#   [pinned-input]    chain tarballs + boot patches + gcc-4.9.4 match sha256.
#   [no-guix]         no /gnu/store bytes in gcc/cc1 (the toolchain placed at /td/store is guix-free).
#   [content-addr]    the interned paths are /td/store/<nar-hash>-<name> (td's own store addressing).
#   [behavioral]      in the own-root, /td/store/<gcc>/bin/gcc compiles+links a static program from
#                     /td/store and it RUNS → 42 — the seed toolchain works from td's own store.
#   [structural]      inside the own-root /td/store IS the store AND /gnu/store is ABSENT (unmixed).
set -eu
...
# --- [pinned-input] extras: chain sources + gawk + glibc-2.16.0 + 2 patches + gcc-4.9.4 -------------
...
echo "   [pinned-input] chain + gawk-3.1.8/glibc-2.16.0/gcc-4.9.4 + the boot patches match their pins"
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BMB"`" "`dirname "$GAWKMB"`" "`dirname "$GOUT"`" "`dirname "$GMB"`" "`dirname "$cpath"`" "${snwork:-/nonexistent}"' EXIT INT TERM

GCCM="$GMB/out"; GO="$GOUT/out"; BU="$BMB/out"
CC1=`ls "$GCCM"/libexec/gcc/i686-unknown-linux-gnu/4.9.4/cc1 2>/dev/null || true`

# --- [no-guix] the toolchain to be placed is guix-free -----------------------------------------------
for b in "$GCCM/bin/gcc" "$CC1"; do
  test -n "$b" -a -f "$b" || fail "gcc-mesboot output missing ($b)"
  if grep -q -a '/gnu/store' "$b"; then fail "$b contains /gnu/store bytes"; fi
done
echo "   [no-guix] no /gnu/store bytes in gcc/cc1 — the toolchain placed at /td/store is guix-free"

# --- intern the static toolchain content-addressed into /td/store ------------------------------------
. tests/cache-lib.sh
...
GCCP=`"$TB" store-add-recursive gcc-mesboot "$GCCM" "$store" "$sndb"` || fail "store-add-recursive gcc-mesboot failed"
BUP=`"$TB" store-add-recursive binutils-mesboot "$BU" "$store" "$sndb"` || fail "store-add-recursive binutils-mesboot failed"
GLP=`"$TB" store-add-recursive glibc-mesboot "$GO" "$store" "$sndb"` || fail "store-add-recursive glibc-mesboot failed"
case "$GCCP" in /td/store/*-gcc-mesboot) ;; *) fail "gcc-mesboot not content-addressed at /td/store: $GCCP" ;; esac
case "$GLP" in /td/store/*-glibc-mesboot) ;; *) fail "glibc-mesboot not content-addressed at /td/store: $GLP" ;; esac
...
gccrel=${GCCP#/td/store/}; burel=${BUP#/td/store/}; glrel=${GLP#/td/store/}
# Compile + link + run inside the own-root (/gnu/store ABSENT): gcc finds cc1/headers relative to
# argv[0] (/td/store/<gcc>/...); as/ld via PATH (binutils); glibc crt+libc via -B + LIBRARY_PATH; the
# read-only /td/store store + a writable tmpfs /tmp for the output.
snscript='
set -e
export PATH=/td/store/'"$burel"'/bin
GCC=/td/store/'"$gccrel"'/bin/gcc; GLIB=/td/store/'"$glrel"'
export LIBRARY_PATH="$GLIB/lib:/td/store/'"$gccrel"'/lib/gcc/i686-unknown-linux-gnu/4.9.4"
export C_INCLUDE_PATH="$GLIB/include"
[ -d /td/store ] && echo TDSTORE-OK
[ -e /gnu/store ] && echo GNU-PRESENT || echo GNU-ABSENT
"$GCC" -static -B"$GLIB/lib" -o /tmp/a.out /td/store/work/hello.c && echo COMPILED
set +e; /tmp/a.out; rc=$?; set -e
echo "RC=$rc"
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns compile+run exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2

echo "$snout" | grep -q '^COMPILED$' || fail "gcc-mesboot at /td/store did not compile a program"
echo "$snout" | grep -q '^RC=42$'    || fail "the /td/store-compiled program did not return 42"
echo "   [behavioral] /td/store/<gcc-mesboot>/bin/gcc compiled+linked a static C program from /td/store and it ran → 42"
echo "$snout" | grep -q '^TDSTORE-OK$' || fail "/td/store is not the store in the own-root"
echo "$snout" | grep -q '^GNU-ABSENT$' || fail "/gnu/store is PRESENT in the own-root — mixed with the guix install!"
echo "   [structural] inside td's own root /td/store IS the store AND /gnu/store is ABSENT (unmixed from guix)"

echo "PASS: source-bootstrap — the seed-built static mesboot toolchain (GCC 4.9.4 + binutils-mesboot +"
echo "      glibc-mesboot 2.16.0) is interned content-addressed at /td/store and COMPILES+RUNS a C"
echo "      program → 42 from /td/store in td's own root with /gnu/store ABSENT. The /td/store-native"
echo "      toolchain: the registered path td-subst can serve, and the unmixed base the userland builds on."
