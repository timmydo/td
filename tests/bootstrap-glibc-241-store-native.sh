#!/bin/sh
# tests/bootstrap-glibc-241-store-native.sh — source-bootstrap BRICK 6/7 (the FINAL modern toolchain, rung C):
...
echo "      MODERN glibc 2.41 (guix's glibc-final). 2.41 is interned at /td/store and gcc 14.3.0 links a"
echo "      DYNAMIC C AND C++ program against it that runs in the own-root → 42, /gnu/store ABSENT. The FULL"
echo "      modern toolchain (gcc 14.3.0 + binutils 2.44 + glibc 2.41) at /td/store, all from the seed."
