#!/bin/sh
# tests/bootstrap-glibc-shared-store-native.sh — source-bootstrap BRICK 6 (first rung): a from-source
# DYNAMIC toolchain at td's OWN store /td/store. From the 229-byte seed, td builds the chain →
# gcc-mesboot1 (4.6.4) + binutils-mesboot, then a SHARED glibc 2.16.0 (libc.so.6 + ld-linux.so.2) — the
# first dynamic artifact. It interns the shared glibc + gcc-mesboot1 + binutils content-addressed into
# /td/store and, in td's own root (rootless userns, /gnu/store ABSENT), links a DYNAMIC C program whose
# interpreter + RUNPATH point at /td/store and RUNS it → 42. This is where /td/store paths first get
# baked into a running dynamic binary — the unmixed-from-guix base the userland is built on.
#
# The shared glibc is guix's glibc-mesboot built --enable-shared. Two non-obvious points, both nailed by
# building guix's REAL glibc-mesboot as a diagnostic oracle (directive 4) and diffing the artifacts:
#  - SKIP the nis subdir: guix's glibc-mesboot ships NO libnsl.so (its lib/ has nss/pthread/rt/crypt/… but
#    not nis) — nis/libnsl.so is exactly where the shared link pulls clnt_gen.o from the non-TLS
#    glibc-mesboot0 (an errno-TLS clash). A dynamic program needs no nis. (Plus the boot patch's
#    libc_hidden_nolink_sunrpc un-hiding; --disable-obsolete-rpc.)
#  - RELOCATION: glibc's libc.so/libpthread.so are ld SCRIPTS with build-time absolute paths; strip them
#    to bare names so ld resolves via -L the /td/store glibc lib dir.
#
# i686, serial. all sources td-fetched; the boot patches are vendored source data; the [no-guix] leg
# verifies no /gnu/store bytes in libc.so.6.
#
# Legs (DURABLE — no guix oracle in any):
#   [pinned-input] chain tarballs + boot patches + glibc-2.16.0 match sha256.
#   [no-guix]      no /gnu/store bytes in the shared libc.so.6.
#   [content-addr] the interned paths are /td/store/<nar-hash>-<name>.
#   [behavioral]   in the own-root, gcc-mesboot1 (@ /td/store) links a DYNAMIC program against the shared
#                  glibc (@ /td/store), its interpreter = the /td/store ld-linux.so.2, and it RUNS → 42.
#   [structural]   inside the own-root /td/store IS the store AND /gnu/store is ABSENT (unmixed from guix).
set -eu
...
echo "   [pinned-input] chain + gawk-3.1.8/glibc-2.16.0 + the boot patches match their pins"
...
# --- [no-guix] the shared libc is guix-free --------------------------------------------------------
if grep -q -a '/gnu/store' "$GLS/lib/libc.so.6"; then fail "$GLS/lib/libc.so.6 contains /gnu/store bytes"; fi
echo "   [no-guix] seed → … → gcc-mesboot1 → SHARED glibc 2.16.0 (libc.so.6 + ld-linux.so.2); no /gnu/store in libc.so.6"

# RELOCATE: glibc's libc.so/libpthread.so are ld SCRIPTS with build-time ABSOLUTE paths → bare names, so
# ld resolves them via -L the /td/store glibc lib dir (the standard glibc relocate; done BEFORE store-add).
for so in "$GLS/lib/"*.so; do
  if head -c20 "$so" 2>/dev/null | grep -q 'GNU ld script' 2>/dev/null; then sed -i "s,$GLS/lib/,,g" "$so"; fi
done

# --- intern the DYNAMIC toolchain content-addressed into /td/store ---------------------------------
. tests/cache-lib.sh
...
# own-root: gcc-mesboot1 (@ /td/store) links a DYNAMIC program against the shared glibc (@ /td/store) with
# INTERP+RUNPATH = /td/store; /td/store ro store + writable tmpfs /tmp; bash builtins only (no grep/cat).
snscript='
set -e
export PATH=/td/store/'"$burel"'/bin
GCC=/td/store/'"$gcrel"'/bin/gcc; GLIB=/td/store/'"$glrel"'
export C_INCLUDE_PATH="$GLIB/include"
export LIBRARY_PATH="$GLIB/lib:/td/store/'"$gcrel"'/lib/gcc/i686-unknown-linux-gnu/4.6.4"
[ -d /td/store ] && echo TDSTORE-OK
[ -e /gnu/store ] && echo GNU-PRESENT || echo GNU-ABSENT
"$GCC" -Wl,--dynamic-linker="$GLIB/lib/ld-linux.so.2" -Wl,-rpath="$GLIB/lib" -B"$GLIB/lib" -o /tmp/d /td/store/work/hello.c && echo COMPILED
re=$(/td/store/'"$burel"'/bin/readelf -l /tmp/d); case "$re" in *"/td/store/'"$glrel"'/lib/ld-linux.so.2"*) echo INTERP-TDSTORE;; esac
set +e; /tmp/d; rc=$?; set -e
echo "RC=$rc"
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns dynamic compile+run exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2

echo "$snout" | grep -q '^COMPILED$'       || fail "gcc-mesboot1 at /td/store did not compile a dynamic program"
echo "$snout" | grep -q '^INTERP-TDSTORE$' || fail "the program's interpreter is NOT the /td/store ld-linux.so.2"
echo "$snout" | grep -q '^RC=42$'          || fail "the dynamic /td/store program did not return 42"
echo "   [behavioral] /td/store/<gcc-mesboot1>/bin/gcc linked a DYNAMIC program against the /td/store shared glibc (interp+RUNPATH = /td/store) and it ran → 42"
echo "$snout" | grep -q '^TDSTORE-OK$' || fail "/td/store is not the store in the own-root"
...
echo "PASS: source-bootstrap brick 6 — from the 229-byte seed, td built a SHARED glibc 2.16.0 and a"
echo "      DYNAMIC program, compiled by the /td/store toolchain, whose interpreter + RUNPATH are /td/store,"
echo "      runs → 42 in td's own root with /gnu/store ABSENT. The dynamic /td/store toolchain — where"
echo "      /td/store is baked into a running binary, unmixed from guix; the base the userland builds on."
