#!/bin/sh
# tests/bootstrap-gcc-mesboot-wrapper.sh — source-bootstrap BRICK 6 (rung 2): a gcc-mesboot-WRAPPER at
# /td/store, the enabling primitive for building real software with the dynamic /td/store toolchain.
# From the 229-byte seed, td builds the chain → gcc-mesboot1 + binutils-mesboot → a SHARED glibc 2.16.0,
# interns them content-addressed into /td/store, and generates a wrapper `gcc` whose ONE job is: a PLAIN
# invocation (no flags — exactly as a real configure/make calls it) produces a DYNAMIC binary against the
# /td/store glibc (interp + RUNPATH = /td/store), with the glibc headers/crt/libc baked in (gcc-mesboot1's
# OWN baked paths are the defunct build-time mktemp dirs). Proven in the store-ns own-root (/gnu/store
# ABSENT): the wrapped gcc, called plainly, compiles a single-file AND a separately-compiled 2-TU program
# → both dynamic, interp=/td/store, run → 42. This is what lets the mesboot userland (bash/coreutils/…)
# and the final modern toolchain build with UNMODIFIED build systems — guix's gcc-mesboot-wrapper, td-native.
#
# i686, serial. all sources td-fetched; the [no-guix] leg verifies no /gnu/store bytes in libc.so.6.
#
...
#   [behavioral]   in the own-root, the WRAPPED gcc — called PLAINLY (no flags) — compiles a single-file
#                  and a 2-TU program; both come out DYNAMIC with interp = the /td/store ld-linux.so.2,
#                  and RUN → 42. (The unmodified-build-system primitive.)
...
mkdir -p "$store/work"
printf '#include <stdio.h>\nint main(){printf("wrapped-dyn\\n");return 42;}\n' > "$store/work/hello.c"
printf 'extern int answer(void);\n#include <stdio.h>\nint main(){printf("multi\\n");return answer();}\n' > "$store/work/m.c"
printf 'int answer(void){return 42;}\n' > "$store/work/a.c"

glrel=${GLP#/td/store/}; gcrel=${GCP#/td/store/}; burel=${BUP#/td/store/}
# Generate the gcc-mesboot-wrapper at /td/store/wrapper/gcc: a PLAIN gcc that bakes everything for a
# /td/store dynamic build (headers, crt -B, libc/libgcc -L, dynamic-linker + rpath).
mkdir -p "$store/wrapper"
cat > "$store/wrapper/gcc" <<WRAP
#!/td/store/$bbase/bin/bash
exec /td/store/$gcrel/bin/gcc \\
  -isystem /td/store/$glrel/include \\
  -B/td/store/$glrel/lib \\
  -L/td/store/$glrel/lib -L/td/store/$gcrel/lib/gcc/i686-unknown-linux-gnu/4.6.4 \\
  -Wl,--dynamic-linker -Wl,/td/store/$glrel/lib/ld-linux.so.2 \\
  -Wl,-rpath -Wl,/td/store/$glrel/lib \\
  "\$@"
WRAP
chmod 0555 "$store/wrapper/gcc"; chmod -R u+w "$store" 2>/dev/null || true
echo "   generated the gcc-mesboot-wrapper at /td/store/wrapper/gcc"

# own-root: the WRAPPED gcc, called PLAINLY, builds dynamic /td/store binaries (single-file + 2-TU).
snscript='
set -e
export PATH=/td/store/'"$burel"'/bin
[ -e /gnu/store ] && echo GNU-PRESENT || echo GNU-ABSENT
/td/store/wrapper/gcc -o /tmp/d /td/store/work/hello.c && echo COMPILED
re=$(/td/store/'"$burel"'/bin/readelf -l /tmp/d); case "$re" in *"/td/store/'"$glrel"'/lib/ld-linux.so.2"*) echo INTERP-TDSTORE;; esac
set +e; /tmp/d; rc=$?; set -e; echo "RC=$rc"
/td/store/wrapper/gcc -c -o /tmp/m.o /td/store/work/m.c
/td/store/wrapper/gcc -c -o /tmp/a.o /td/store/work/a.c
/td/store/wrapper/gcc -o /tmp/multi /tmp/m.o /tmp/a.o && echo MULTI-LINKED
set +e; /tmp/multi; mrc=$?; set -e; echo "MRC=$mrc"
'
snout=`"$TB" store-ns "$store" -- "/td/store/$bbase/bin/bash" -c "$snscript" 2>&1` || { printf '%s\n' "$snout" | sed 's/^/     /' >&2; fail "store-ns wrapped build+run exited nonzero"; }
printf '%s\n' "$snout" | sed 's/^/     /' >&2

echo "$snout" | grep -q '^COMPILED$'       || fail "the plain wrapped gcc did not compile a single-file program"
echo "$snout" | grep -q '^INTERP-TDSTORE$' || fail "the wrapped-gcc output interpreter is NOT the /td/store ld-linux.so.2"
echo "$snout" | grep -q '^RC=42$'          || fail "the wrapped-gcc dynamic program did not return 42"
echo "$snout" | grep -q '^MULTI-LINKED$'   || fail "the wrapped gcc could not separately compile + link 2 objects"
echo "$snout" | grep -q '^MRC=42$'         || fail "the multi-TU wrapped program did not return 42"
echo "   [behavioral] the /td/store gcc-mesboot-wrapper, called PLAINLY, built a single-file AND a 2-TU program → both dynamic (interp = /td/store ld-linux.so.2), ran → 42"
...
echo "PASS: source-bootstrap brick 6 (rung 2) — from the 229-byte seed, td built a SHARED glibc 2.16.0 and"
echo "      a gcc-mesboot-WRAPPER at /td/store: a PLAIN gcc invocation (no flags, as a real build system"
echo "      calls it) produces DYNAMIC /td/store binaries that run → 42, /gnu/store ABSENT. The primitive"
echo "      that lets the mesboot userland + the final modern toolchain build with unmodified build systems."
