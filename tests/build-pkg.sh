...
# ONCE more and compares that against the build already realized — 2 single-threaded
# builds per package (the reproducibility proof needs two INDEPENDENT builds, not two
# fresh throwaways; the daemon CHECK verb reuses the build it just produced as the first
# leg). The builder runs make serially (NIX_BUILD_CORES=1), so `build-recipes` can run
# ~nproc of these at once with no internal oversubscription. The check gates then
# cache-hit the build and memo-skip the repro rebuild, so they only run their durable
# behavioral + migration-oracle assertions.
#
...
# only a CHANGED recipe (⇒ different drv ⇒ miss) does the 2 builds. Verbose output goes
