#!/bin/sh
# tests/bootstrap-glibc-mesboot.sh — source-bootstrap BRICK 5 (gcc toolchain): the MODERN C library,
# GNU libc 2.16.0 — guix's glibc-mesboot. After glibc-mesboot0 (2.2.5, the bootstrap libc), the
# gcc-mesboot1 (GCC 4.6.4, C++) toolchain + binutils-mesboot + gawk-mesboot build glibc 2.16.0 with nptl
# threads. This is the C library the final mesboot gcc (gcc-mesboot, GCC 4.9) is built against.
#
# From the 229-byte seed, td builds the whole chain (bricks 0-4 + make/patch/binutils/gcc/glibc rungs +
# gcc-mesboot0 + binutils-mesboot1 + make-mesboot + gcc-mesboot1 + binutils-mesboot + gawk-mesboot), then
# builds glibc 2.16.0 in two stages in one source tree: (A) the bootstrap headers, (B) the full nptl
# library. CC=<gcc-mesboot1>; the BUILD tools (sunrpc/cross-*) get glibc-mesboot0 + kernel headers via
# C_INCLUDE_PATH/LIBRARY_PATH (target objects use -nostdinc, so unaffected).
#
# td builds glibc 2.16.0 STATIC (guix builds it shared): td's whole chain is static + glibc-mesboot0 is
# --disable-shared/--without-tls, and a shared build made the new libnsl.so fall back to that old static
# libc for sunrpc symbols (errno TLS-vs-non-TLS clash). --disable-shared drops the shared libs entirely.
# Three library-only tweaks fall out: drop the nscd PROGRAM + the texinfo `manual` subdir (both pull nss/
# makeinfo that can't link/run here; the nss client objects in libc are kept), and create an empty
# soversions.mk so `make install` (which lists it) proceeds.
#
# i686, static, serial. all sources td-fetched; the 7 guix boot patches are vendored source data —
# libc.a is compiled from glibc source (no guix bytes; the [no-guix] leg verifies).
#
# Legs (DURABLE):
#   [pinned-input] chain tarballs + 7 vendored boot patches + …/gawk-3.1.8/glibc-2.16.0 match sha256.
#   [no-guix]      built on a curated PATH with gcc/g++/cc/guile/guix DENIED; no /gnu/store in libc.a.
#   [behavioral]   gcc-mesboot1 links a C program AND a pthread (nptl) program against the new glibc
#                  2.16.0 statically — both RUN → 42 — a modern, threaded C library from source.
#   [repro]        crt objects byte-identical AND a static (nptl) program linked against the libc
#                  reproduces byte-for-byte across two builds (the .a archives carry only ar mtimes).
set -eu
...
# --- [pinned-input] extras: the gcc-mesboot1 chain sources + gawk-3.1.8 + glibc-2.16.0 + its 2 patches -
...
echo "   [pinned-input] + gcc-4.6.4/gcc-g++/gmp/mpfr/mpc/gawk-3.1.8/glibc-2.16.0 + the gcc-4.6.4 & 2 glibc-2.16.0 boot patches match their pins"
...
trap 'rm -rf "$tc" "$mesp" "`dirname "$TCCD"`" "`dirname "$MK"`" "`dirname "$PD"`" "`dirname "$BD"`" "`dirname "$GD"`" "`dirname "$HD"`" "`dirname "$GLD"`" "`dirname "$G2"`" "`dirname "$B2"`" "`dirname "$MM"`" "`dirname "$GM1"`" "`dirname "$BMB"`" "`dirname "$GAWKMB"`" "`dirname "$GOUT"`" "`dirname "$cpath"`"' EXIT INT TERM

GO="$GOUT/out"; LIBC="$GO/lib/libc.a"; GM1DIR="$GM1/out/lib/gcc/i686-unknown-linux-gnu/4.6.4"

# --- [no-guix] -------------------------------------------------------------------------------
test -s "$LIBC" -a -s "$GO/lib/libpthread.a" -a -f "$GO/lib/crt1.o" || fail "glibc 2.16.0 missing libc.a/libpthread.a/crt"
if grep -q -a '/gnu/store' "$LIBC"; then fail "$LIBC contains /gnu/store bytes"; fi
echo "   [no-guix] seed → … → gcc-mesboot1 → {binutils-mesboot, gawk-mesboot} → glibc 2.16.0 built with no gcc/guile/guix on PATH; no /gnu/store in libc.a"

# --- [behavioral] gcc-mesboot1 links a C program AND a pthread (nptl) program against glibc 2.16.0 → 42
wd=`mktemp -d`
printf '#include <stdio.h>\nint main(){printf("hi");return 42;}\n' > "$wd/t.c"
( cd "$wd" && env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" LIBRARY_PATH="$GO/lib:$GM1DIR" \
    "$GM1/out/bin/gcc" -static -B"$GO/lib" -o t t.c ) >"$wd/cc.log" 2>&1 \
  || { tail -12 "$wd/cc.log" >&2; rm -rf "$wd"; fail "could not link a C program against glibc 2.16.0"; }
head -c20 "$wd/t" | od -An -tx1 | grep -q '7f 45 4c 46 01' || { rm -rf "$wd"; fail "not a 32-bit ELF"; }
set +e; ( cd "$wd" && env -i ./t ); rcc=$?; set -e
test "$rcc" = 42 || { rm -rf "$wd"; fail "the glibc-2.16.0-linked C program returned $rcc, want 42"; }
printf '#include <pthread.h>\nstatic void* f(void* a){return a;}\nint main(){pthread_t t;pthread_create(&t,0,f,0);pthread_join(t,0);return 42;}\n' > "$wd/p.c"
( cd "$wd" && env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" LIBRARY_PATH="$GO/lib:$GM1DIR" \
    "$GM1/out/bin/gcc" -static -B"$GO/lib" -o p p.c -lpthread ) >"$wd/p.log" 2>&1 \
  || { tail -12 "$wd/p.log" >&2; rm -rf "$wd"; fail "could not link a pthread program against glibc 2.16.0 (nptl)"; }
set +e; ( cd "$wd" && env -i ./p ); rcp=$?; set -e
rm -rf "$wd"
test "$rcp" = 42 || fail "the glibc-2.16.0 pthread program returned $rcp, want 42"
echo "   [behavioral] gcc-mesboot1 linked a C program → 42 AND a pthread (nptl) program → 42 against the new static glibc 2.16.0 — a modern, threaded C library from source"

# --- [repro] crt objects byte-identical AND the libc's linked output is byte-identical ---------------
# The libc.a/libpthread.a archives carry non-deterministic `ar` member mtimes (binutils 2.20.1a), but the
# OBJECTS are deterministic. Assert the durable property: the crt objects reproduce byte-for-byte AND a
# static (nptl) program linked against the libc reproduces byte-for-byte (ld pulls objects, not mtimes).
r1=`sha "$GO/lib/crt1.o"`; i1=`sha "$GO/lib/crti.o"`; n1=`sha "$GO/lib/crtn.o"`
rs=`mktemp -d`; printf '#include <stdio.h>\n#include <pthread.h>\nstatic void* f(void* a){return a;}\nint main(){pthread_t t;pthread_create(&t,0,f,0);pthread_join(t,0);printf("x");return 7;}\n' > "$rs/r.c"
env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" LIBRARY_PATH="$GO/lib:$GM1DIR" "$GM1/out/bin/gcc" -static -B"$GO/lib" -O2 -o "$rs/a" "$rs/r.c" -lpthread || { rm -rf "$rs"; fail "could not link the repro program"; }
a1=`sha "$rs/a"`
build_glibc_mesboot "$cpath" "$GM1" "$BMB" "$GAWKMB" "$GLD" "$MM" "$PD" "$GOUT" || { rm -rf "$rs"; fail "the second glibc 2.16.0 build did not run"; }
test "$r1" = "`sha "$GO/lib/crt1.o"`" -a "$i1" = "`sha "$GO/lib/crti.o"`" -a "$n1" = "`sha "$GO/lib/crtn.o"`" || { rm -rf "$rs"; fail "glibc 2.16.0 crt objects are NOT reproducible"; }
env -i PATH="$BMB/out/bin:$cpath" C_INCLUDE_PATH="$GO/include" LIBRARY_PATH="$GO/lib:$GM1DIR" "$GM1/out/bin/gcc" -static -B"$GO/lib" -O2 -o "$rs/b" "$rs/r.c" -lpthread || { rm -rf "$rs"; fail "could not link against the rebuilt libc"; }
test "$a1" = "`sha "$rs/b"`" || { rm -rf "$rs"; fail "the libc's linked output is NOT reproducible across builds"; }
rm -rf "$rs"
echo "   [repro] crt objects are byte-identical AND a static (nptl) program linked against the libc reproduces byte-for-byte across two independent builds (intrinsic output-reproducibility; the .a archives carry only ar member mtimes)"

echo "PASS: source-bootstrap brick 5 — from the 229-byte seed, td built GNU libc 2.16.0 (guix's"
echo "      glibc-mesboot): a modern, nptl-threaded C library built by gcc-mesboot1 + binutils-mesboot +"
echo "      gawk-mesboot, that links a C program AND a pthread program → 42; no gcc/guile/guix, no"
echo "      /gnu/store, reproducible. gcc-mesboot (GCC 4.9, the final mesboot gcc) is next."
