...
/// The committed artifact. The face is emitted as a Rust module of hex text
/// rather than as the raw PSF2, because the target recipe stages module
/// sources through `Step::WriteFile`, whose content is a `String` -- a binary
/// asset beside them has nothing to carry it, and `include_bytes!` of one
...
fn run() -> Result<(), String> {
    ...
    let digest_out = sha256::hex_digest(&face);
    write_atomically(&out_dir.join(MODULE_FILE), module(&face, &digest_out).as_bytes())?;
    println!(
        "{MODULE_FILE}: {} single-width glyphs, {ASSET_FILE} would be {} bytes, sha256 {digest_out}",
        glyphs.len(),
        face.len(),
    );
...
/// The face as a module of hex text. One pair per byte keeps it diffable and
/// lets the reader rebuild the exact PSF2 the digest names.
fn module(face: &[u8], digest: &str) -> String {
    let mut hex = String::with_capacity(face.len().saturating_mul(2));
    for byte in face {
        hex.push_str(&format!("{byte:02x}"));
    }
    let mut out = String::new();
    out.push_str(&format!("//! Generated by tools/import-unifont.rs from {ARCHIVE_URL}.\n"));
    out.push_str("//! Do not edit: re-run the importer. See assets/PROVENANCE.\n\n");
    out.push_str(&format!("/// The pinned {RELEASE} face, hex-encoded. Text rather than a\n"));
    out.push_str("/// binary so the target recipe can stage it as an ordinary module.\n");
    out.push_str(&format!("/// Decodes to the PSF2 whose SHA-256 is {digest}.\n"));
    out.push_str(&format!("pub const UNIFONT_HEX: &str = \"{hex}\";\n"));
    out
}
...
mod tests {
    ...
    const GENERATED: &str = include_str!("../src/font_data.rs");
    ...
    fn the_committed_asset_is_the_bytes_provenance_describes() {
        // The generated module is text, so its claim is checked by decoding
        // the hex it carries rather than by importing it.
        assert!(GENERATED.contains(ASSET_SHA256), "module omits its digest");
        let hex = GENERATED
            .split_once("UNIFONT_HEX: &str = \"")
            .and_then(|(_, tail)| tail.split_once('"'))
            .map(|(hex, _)| hex)
            .expect("generated module carries a hex literal");
        let face = decode(hex);
        assert_eq!(face.len(), 422_671);
        assert_eq!(sha256::hex_digest(&face), ASSET_SHA256);
        for claim in [
            ASSET_SHA256,
            ARCHIVE_SHA256,
            HEX_SHA256,
