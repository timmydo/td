//! Linux x86-64 named credential intake; separate from the compositor channel.
...
fn adopt(fd: i32) -> OwnedFd {
    // SAFETY: only a successful memfd result or a freshly installed ancillary
    // descriptor reaches this site; never an integer from message-body bytes.
...
pub(super) fn prepare_receiver(stream: &UnixStream) -> io::Result<()> {
    let enabled: i32 = 1;
...
fn admit(
    ...
    control_len: usize,
    flags: i32,
    records: (Option<Credentials>, Vec<OwnedFd>, Vec<OwnedFd>, bool),
) -> io::Result<(usize, Sender)> {
    let (credentials, mut pidfds, mut rights, valid) = records;
    if count == 0 {
    ...
    if pidfds.len() != 1 || rights.len() > 1 {
        return Err(io::Error::other(
            "credential intake needs one sender pidfd and at most one credential descriptor",
        ));
    }
    let pidfd = pidfds
        .pop()
    ...
    Ok((
        ...
        Sender {
            credentials,
            pidfd,
            descriptor: rights.pop().map(File::from),
        },
    ))
}
...
fn harvest(bytes: &[u8]) -> (Option<Credentials>, Vec<OwnedFd>, Vec<OwnedFd>, bool) {
    ...
    (credentials, pidfds, rights, valid)
}

pub(super) fn create_credential() -> io::Result<File> {
    let raw = result(syscall5(
        SYS_MEMFD_CREATE,
        c"td-secret-set".as_ptr() as usize,
        MEMFD_FLAGS,
        0,
        0,
        0,
    ...
    let raw = i32::try_from(raw)
        .map_err(|_| io::Error::other("invalid created credential descriptor"))?;
    Ok(File::from(adopt(raw)))
}

pub(super) fn seal_credential(file: &File) -> io::Result<()> {
    result(syscall5(
        SYS_FCNTL,
...
pub(super) fn require_sealed(file: &File) -> io::Result<()> {
    let seals = result(syscall5(
        SYS_FCNTL,
        file.as_raw_fd() as usize,
        F_GET_SEALS,
        0,
    ...
    if seals & REQUIRED_SEALS != REQUIRED_SEALS {
        return Err(io::Error::other("credential descriptor is not immutable"));
    }
    Ok(())
}

pub(super) fn send_descriptor(stream: &UnixStream, bytes: &[u8], file: &File) -> io::Result<()> {
    if bytes.is_empty() || bytes.len() > 256 {
        return Err(io::Error::other("invalid credential request length"));
    }
    let mut control = Control([0; CONTROL]);
    for (at, value) in [
        (0, 20usize.to_ne_bytes().as_slice()),
        (8, (SOL_SOCKET as i32).to_ne_bytes().as_slice()),
        (12, SCM_RIGHTS.to_ne_bytes().as_slice()),
        (16, file.as_raw_fd().to_ne_bytes().as_slice()),
    ] {
        control
            .0
            .get_mut(at..at + value.len())
            .ok_or_else(|| io::Error::other("invalid send control field"))?
    ...
    if sent == 0 || sent > bytes.len() {
        return Err(io::Error::other("invalid credential send count"));
    }
    let mut stream = stream;
    stream.write_all(
        bytes
            .get(sent..)
            .ok_or_else(|| io::Error::other("invalid credential send cursor"))?,
    )
}

#[cfg(test)]
#[path = "../tests/secret_sys.rs"]
mod tests;
