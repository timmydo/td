...
pub fn sine_q15(phase: u32) -> i32 {
    ...
    let interpolated =
        i64::from(low) + ((i64::from(high - low) * fraction) >> PHASE_FRACTION_BITS);
...
impl Tone {
    ...
    pub const fn fixture() -> Self {
        Self { hertz: 440, amplitude: 10000 }
    }
}
...
mod tests {
    ...
    fn the_table_is_a_quarter_sine() {
        ...
        for pair in QUARTER_SINE.windows(2) {
            let (a, b) = (pair.first().copied().unwrap(), pair.last().copied().unwrap());
    ...
    fn the_closed_form_matches_the_running_generator() {
        let spec = Spec::fixed();
        let tone = Tone { hertz: 997, amplitude: 20000 };
        let mut generator = Generator::new(spec, tone);
        let mut out = Vec::new();
        ...
        for frame in 0..200u64 {
            ...
            assert_eq!(got, Generator::sample_at(spec, tone, frame), "frame {frame}");
        }
    ...
    fn the_phase_step_is_the_frequency_ratio() {
        ...
        assert_eq!(phase_step(48000, 48000), 0, "a full cycle per frame wraps to zero");
