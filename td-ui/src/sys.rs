...
/// owns the returned descriptors until an event's consumer takes them.
pub(crate) fn receive(stream: &UnixStream, bytes: &mut [u8]) -> io::Result<(usize, Vec<OwnedFd>)> {
    if bytes.is_empty() {
...
pub(crate) fn send_file(stream: &UnixStream, bytes: &[u8], file: &File) -> io::Result<usize> {
    if bytes.is_empty() {
        return Err(io::Error::other("empty descriptor request"));
    }
    let mut control = Control([0; CONTROL]);
    // cmsg_len is a size_t: the 16-byte header plus one descriptor.
    if let Some(slot) = control.0.get_mut(..8) {
        slot.copy_from_slice(&20usize.to_ne_bytes());
    }
    for (offset, value) in [
        (8, SOL_SOCKET as u32),
