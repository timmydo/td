...
HEAVY_RUNGS := rollback generation-image no-guix manifest-check oci container rootless oci-load registry verify-place reset test place build boot-disk td-builder run offline memo ts ts-eval ts-diff corpus corpus-deps td-build td-build-deps drv-emit td-drv-build td-drv-add td-drv-assemble td-check resolve td-build-resolved loop-sandbox loop-rung store-register store-add store-add-tree store-gc store-verify store-gc-sweep
...
# This rung is the FIRST decoupling step, done the loop-sandbox/td-check way —
...
# (tests/td-build-inputs.lock) with NO Guile, and the rung proves td's lock
...
# (as the existing store-add/drv-add rungs do) purely as the oracle — host infra stays
...
store-add-tree:
	...
	printf '%s\n' \
	  ...
	  > "$$scratch/lower.scm"; \
	src=`$(GUIX) repl $(LOAD) "$$scratch/lower.scm"`; \
