...
#     then driven with SYNTHETIC identities + a scratch verdict dir, so this
#     rung behaves identically on dev hosts and CI and never touches the real
#     .check-verdicts state.
...
memo:
	...
	out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-one sh tests/check-memo.sh "$$det" 2>&1`; \
	...
	out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-one sh tests/check-memo.sh "$$det2" 2>&1`; \
	...
	out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-two sh tests/check-memo.sh "$$det" 2>&1`; \
	...
	out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-two TD_CHECK_FULL=1 sh tests/check-memo.sh "$$det" 2>&1`; \
	...
	out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV= sh tests/check-memo.sh "$$det" 2>&1`; \
	...
	if out=`TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-two TD_CHECK_TTL_DAYS=15 sh tests/check-memo.sh "$$det" 2>&1`; then \
	...
	if TD_GUIX="$(GUIX)" TD_CHECK_VERDICTS="$$vd" TD_CHECK_ENV=td-memo-env-one sh tests/check-memo.sh "$$nondet" > "$$vd/nondet.log" 2>&1; then \
