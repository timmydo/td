...
srcinfo=`sh tests/intern-src.sh "$tb" td-cmake-demo-src "$PWD/tests/cmake-demo" "$scratch"` || { echo "ERROR: td could not intern the cmake-demo tree (store-add-recursive)" >&2; exit 1; }; \
eval "$srcinfo"; \
...
env -i HOME="$scratch" TMPDIR="$scratch/tmp" PATH="$cu/bin" TD_BUILDER_PATH="$TD_BUILDER_PATH" TD_BUILDER_STORE="$TD_BUILDER_STORE" TD_BUILDER_DB="$TD_BUILDER_DB" "$tb" build-recipe "$scratch/td-cmake-demo.json" "$lock" "$sd" /gnu/store "$srcstore" "$srcdb" > "$scratch/bout" 2>"$scratch/err" || { echo "FAIL: build-recipe cmake build (guix/Guile off PATH):" >&2; tail -30 "$scratch/err" >&2; exit 1; }; \
...
{ echo "(use-modules (guix packages) (guix gexp) (guix build-system cmake) ((guix licenses) #:prefix license:))"; \
  ...
  echo "  (source (local-file \"$PWD/tests/cmake-demo\" \"td-cmake-demo-src\" #:recursive? #t))"; \
...
gdrv=`$TD_GUIX build -d -f "$oracle" 2>/dev/null` || { echo "ERROR: could not compute the guix cmake-build-system oracle derivation (warm its closure on a channel bump)" >&2; exit 1; }; \
gout=`printf '(use-modules (guix derivations))\n(for-each (lambda (o) (display (derivation-output-path (cdr o))) (newline)) (derivation-outputs (read-derivation-from-file "%s")))\n' "$gdrv" | $TD_GUIX repl 2>/dev/null | grep -oE '/gnu/store/[a-z0-9]+-td-cmake-demo-guix-[^ ]+' | head -1` || true; \
