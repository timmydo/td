...
pub fn gate() -> GateDef {
    GateDef {
        ...
        inputs: &[ArtifactInput {
            name: "coreutils",
            kind: InputKind::LockEntry { lock: "tests/td-russh-demo.lock", stem: "coreutils" },
        }],
...
set -euo pipefail; \
vendor="$PWD/.td-build-cache/crate-vendor/russh"; \
...
clock="$PWD/tests/russh-demo/Cargo.lock"; \
...
lock0="$PWD/tests/td-russh-demo.lock"; \
...
scratch="$PWD/.td-build-cache/rust-russh"; rm -rf "$scratch"; mkdir -p "$scratch/tmp" "$scratch/sd"; \
...
srcinfo=`sh tests/intern-src.sh "$tb" td-russh-demo-src "$PWD/tests/russh-demo" "$scratch/src" target .cargo` || { echo "ERROR: td could not intern the russh-demo crate tree (store-add-recursive)" >&2; exit 1; }; \
eval "$srcinfo"; \
