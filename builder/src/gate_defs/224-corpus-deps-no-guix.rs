...
lkh=`for p in $($TD_GUIX build linux-libre-headers 2>/dev/null); do [ -f "$p/include/linux/limits.h" ] && echo "$p/include" && break; done`; \
test -n "$lkh" || { echo "ERROR: could not resolve linux-libre-headers for the link-test" >&2; exit 1; }; \
ncs=`for p in $($TD_GUIX build ncurses 2>/dev/null); do [ -f "$p/lib/libncurses.so" ] && echo "$p/lib" && break; done`; \
...
for spec in libsigsegv libunistring pcre2 ncurses readline; do \
  echo "================ $spec ================"; \
  lock="$PWD/tests/$spec-no-guix.lock"; \
  test -s "$lock" || { echo "ERROR: no lock $lock" >&2; exit 1; }; \
  grep ' /gnu/store/' "$lock" | sed 's/^[^ ]* //' | xargs $TD_GUIX build >/dev/null || { echo "ERROR: could not realize the seed for $spec" >&2; exit 1; }; \
