...
pub fn gate() -> GateDef {
    GateDef {
        ...
        script: r##"
echo ">> stage0-cold-start: a COLD stage0 placement works with guix state HIDDEN — same path, same closure as the warm guix-host placement; an absent seed dir places with no refs (#313)"
set -euo pipefail; \
...
echo ">> warm leg (baseline): place via the shared base with guix state present"; \
cbw=`sh tests/stage0-builder.sh "$PWD/.td-build-cache/stage0"` || { echo "FAIL: warm stage0 placement failed" >&2; exit 1; }; \
tbw="$PWD/.td-build-cache/stage0/store/`basename "$cbw"`/bin/td-builder"; \
test -x "$tbw" || { echo "FAIL: warm stage0 not executable at $tbw" >&2; exit 1; }; \
...
"$tbw" store-closure "$PWD/.td-build-cache/stage0/builder.db" "$cbw" | sort > "$scratch/cl.warm"; \
...
"$tbw" store-closure "$scratch/pb.db" "$pb" | grep -qF "$g" \
