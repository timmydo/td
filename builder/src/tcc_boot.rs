...
/// includes it for `TCC_VERSION`). The generated `config.mak` is written for
/// fidelity but the hand-build sets its own flags and never sources it. With
/// `cc=mescc` configure detects no gcc, so `GCC_MAJOR`/`GCC_MINOR` are empty.
...
fn config_defs(cfg: &Cfg) -> Vec<String> {
    let out = &cfg.out;
    let mes = ps(&cfg.mes).unwrap_or("");
    vec![
        "-D".into(),
        "inline=".into(),
        "-D".into(),
