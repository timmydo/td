...
//! every REGISTERED gate script body.
//!
...
/// Every registered gate body, scanned. Returns all findings (empty when the tree
/// is clean). The single entry point the gate/test drives.
...
mod tests {
    ...
    fn flags_comment_spliced_into_a_continuation() {
        ...
        assert!(
            f.iter()
                .any(|m| m.contains("fixture:2") && m.contains("eats the continuation")),
    ...
    fn flags_comment_line_ending_in_backslash() {
        ...
        assert!(
            f.iter()
                .any(|m| m.contains("fixture:2") && m.contains("ends in `\\`")),
    ...
    /// The live guard: every registered gate body is clean. This
