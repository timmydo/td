...
use crate::checks::qemu_boot::{QEMU_USER_NETDEV, QEMU_USER_NET_DEVICE, SYSTEM_GUEST_MEMORY_MIB};
...
pub(crate) fn launcher_script(format: DiskFormat) -> String {
    launcher_script_mode(format, false)
}

pub(crate) fn launcher_script_mode(format: DiskFormat, installation: bool) -> String {
    let mut out = String::new();
    ...
    out.push_str(
        "#!/bin/sh\n\
         # td VM launcher. GENERATED by `td-recipe-eval bundle` from\n\
    ...
    let _ = writeln!(out, "memory=${{{MEMORY_ENV}:-{SYSTEM_GUEST_MEMORY_MIB}}}");
    out.push_str(
        "\n\
         usage() {\n\
         \x20   cat <<'USAGE'\n\
",
    ...
    out.push_str(
        "         \n\
         Boots the td system under QEMU. The guest's serial console is wired\n\
...
",
    ...
    out.push_str(if installation {
        "All guest writes persist in this installation disk.\n\n"
    } else {
...
",
    ...
    let _ = writeln!(out, "persist={}", u8::from(installation));
    out.push_str(
        "while [ $# -gt 0 ]; do\n\
         \x20   case $1 in\n\
...
mod tests {
    ...
    fn the_launcher_carries_every_profile_token() {
        ...
        assert!(
            ...
            "launcher dropped the kernel cmdline"
        );
        assert!(
            script.contains(DISK_DEVICE),
            "launcher dropped the disk device"
        );
        assert!(
    ...
    fn the_rendered_launcher_parses_as_posix_sh() {
        ...
        for (format, installation) in [
            (DiskFormat::Qcow2(Compression::Zstd), false),
            (DiskFormat::Raw, false),
            (DiskFormat::Qcow2(Compression::Zstd), true),
            (DiskFormat::Raw, true),
        ] {
            let script = launcher_script_mode(format, installation);
    ...
    fn every_site_that_names_the_blockdev_uses_one_id() {
        ...
        let rust_drive =
            crate::checks::qemu_boot::drive_arg(std::path::Path::new("volume.btrfs"), false);
        assert!(
            rust_drive
                .to_string_lossy()
                .contains(&format!("id={DRIVE_ID},")),
    ...
    fn the_cluster_size_prose_matches_the_flag_the_image_was_written_with() {
        for &compression in Compression::ALL {
            ...
            let (value, unit) = label.split_at(label.find(' ').unwrap_or(label.len()));
