    ...
    steps.push(
        Step::run(
            ...
            &[
                SH,
                "-c",
                "for a in sh ls sed grep awk tar gzip cat echo mkdir rm cp true false env printf sleep sort head tail basename dirname mktemp tee touch tr test pwd; do ln -sf busybox \"$a\"; done",
            ],
        )
        .env("PATH", &base_path()),
    );
