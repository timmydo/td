...
// that guard rejects (use `position`/`match_indices` and plain loops over the
// search combinator that shares a name with one) — they would trip the
// host-tool-tier guard even though rustc never interprets the file as a shell
// script. Same constraint td-sh/td-kexec/td-netd/td-util/td-init document.
