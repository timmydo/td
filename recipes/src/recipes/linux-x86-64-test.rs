...
pub fn recipe() -> Recipe {
    ...
    steps.push(
        Step::run(
            ...
            &[
                ...
                "-c",
                &format!(
                    "grep -q -a 'Linux version 4.14.67' '{vmlinux}' || {{ echo 'vmlinux is missing the Linux 4.14.67 banner' >&2; exit 1; }}"
                ),
            ],
        )
        .env("PATH", &mesboot0_path()),
    ...
    steps.push(Step::WriteFile {
        ...
        content: "PASS: Linux 4.14.67 vmlinux, source-built by the native /td/store x86_64 toolchain, is a well-formed ELF64 x86-64 image carrying the Linux banner\n".into(),
        exec: false,
    });
    ...
    Recipe::mesboot("linux-x86-64-test", "1.0")
        ...
        .checks(vec![RecipeCheck::daily(
            r#"
echo ">> recipe-check linux-x86-64-test: build-plan --auto builds linux-x86-64 (Linux 4.14.67 vmlinux, source-built by the native /td/store x86_64 GCC 14 + glibc 2.41 toolchain) and asserts a well-formed ELF64 x86-64 image with the Linux banner"
