...
const XBIN: &str = "{in:binutils-x86-64}/bin";
const XGLIBC: &str = "{in:glibc-x86-64}/stage/td/store/glibc-2.41-x86_64";

pub fn recipe() -> Recipe {
    Recipe::mesboot("gcc-x86-64-stage2-test", "1.0")
        ...
        .steps(vec![
            ...
            compile_step(XGCC, "{root}/test/assert.c", "{root}/test/assert-tool", false),
            compile_step(XGCC, "{root}/test/probe.c", "{root}/test/probe-c", false),
            compile_step(XGPP, "{root}/test/probe.cc", "{root}/test/probe-cxx", true),
            ...
            Step::run(
                ...
                &[
                    "{root}/test/assert-tool",
                    "no-gnu-tree",
                    "{in:gcc-x86-64-stage2}/stage/td/store/gcc-14.3.0-x86_64/libexec/gcc/x86_64-pc-linux-gnu/14.3.0/cc1",
                ],
            ),
            Step::run(
                "{root}/test",
                &[
                    "{root}/test/assert-tool",
                    "no-gnu-tree",
                    "{in:gcc-x86-64-stage2}/stage/td/store/gcc-14.3.0-x86_64/libexec/gcc/x86_64-pc-linux-gnu/14.3.0/cc1plus",
                ],
            ),
            Step::run(
                "{root}/test",
            ...
            Step::WriteFile {
                ...
                content: "gcc-x86-64-stage2 recipe test passed\n".into(),
                exec: false,
            },
            Step::Require {
        ...
        .checks(vec![RecipeCheck::daily(
            r#"
echo "[td] running gcc-x86-64-stage2-test recipe check"
