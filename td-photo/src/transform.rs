...
//! `av1_fwd_txfm1d.c`; the tests hold them to a floating-point reference.
...
pub enum Op {
    ...
    /// `-x[i]`.
    Neg(u8),
    /// `x[i] + x[j]`, clamped in the inverse.
    Add(u8, u8),
    /// `x[i] - x[j]`, clamped in the inverse.
    Sub(u8, u8),
    /// `Round2(cos(a) * x[i] + cos(b) * x[j], bits)`, a negative index
    /// for a negated cosine (spec `B()`).
    Btf(i8, u8, i8, u8),
}

use Op::{Add as A, Btf as B, Copy as C, Neg as N, Sub as S};
...
/// Runs a butterfly table over `x` in place, `clamp` for the inverse's
/// intermediate clamp; a slice of another length is left as it is.
fn butterflies<const N: usize>(
    ...
    let Ok(x) = <&mut [i32; N]>::try_from(x) else {
        return;
    };
    let at = |v: &[i32; N], i: u8| v.get(usize::from(i)).copied().unwrap_or(0);
    let mut cur = *x;
    for stage in stages {
        let mut next = [0i32; N];
        for (out, op) in next.iter_mut().zip(stage.iter()) {
            let v = match *op {
                Op::Copy(i) => at(&cur, i),
                Op::Neg(i) => -at(&cur, i),
                Op::Add(i, j) => at(&cur, i) + at(&cur, j),
                Op::Sub(i, j) => at(&cur, i) - at(&cur, j),
                Op::Btf(a, i, b, j) => round2(
                    i64::from(cosine(table, a)) * i64::from(at(&cur, i))
                        + i64::from(cosine(table, b)) * i64::from(at(&cur, j)),
                    bits,
                ),
            };
            *out = if clamp && matches!(op, Op::Add(..) | Op::Sub(..)) {
                clamp16(v)
            } else {
                v
            };
...
fn inverse_1d(kernel: Kernel, x: &mut [i32]) {
    match (kernel, x.len()) {
        (Kernel::Dct, 4) => butterflies(IDCT4, x, &COSPI12, INVERSE_BITS, true),
        (Kernel::Dct, 8) => butterflies(IDCT8, x, &COSPI12, INVERSE_BITS, true),
        (Kernel::Dct, 16) => butterflies(IDCT16, x, &COSPI12, INVERSE_BITS, true),
        (Kernel::Dct, 32) => butterflies(IDCT32, x, &COSPI12, INVERSE_BITS, true),
        (Kernel::Adst, 4) => iadst4(x),
        (Kernel::Adst, 8) => butterflies(IADST8, x, &COSPI12, INVERSE_BITS, true),
        (Kernel::Adst, 16) => butterflies(IADST16, x, &COSPI12, INVERSE_BITS, true),
        _ => {}
    }
}
...
fn forward_1d(kernel: Kernel, x: &mut [i32]) {
    match (kernel, x.len()) {
        (Kernel::Dct, 4) => butterflies(FDCT4, x, &COSPI13, FORWARD_BITS, false),
        (Kernel::Dct, 8) => butterflies(FDCT8, x, &COSPI13, FORWARD_BITS, false),
        (Kernel::Dct, 16) => butterflies(FDCT16, x, &COSPI13, FORWARD_BITS, false),
        (Kernel::Dct, 32) => butterflies(FDCT32, x, &COSPI13, FORWARD_BITS, false),
        (Kernel::Adst, 4) => fadst4(x),
        (Kernel::Adst, 8) => butterflies(FADST8, x, &COSPI13, FORWARD_BITS, false),
        (Kernel::Adst, 16) => butterflies(FADST16, x, &COSPI13, FORWARD_BITS, false),
        _ => {}
    }
}
